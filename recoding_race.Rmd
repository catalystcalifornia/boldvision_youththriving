---
title: "Recoding Race"
author: "Alicia Vo"
date: "2024-09-20"
output: html_document
---
Pull data from database.
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(data.table)
library(dplyr)
library(RPostgreSQL)
library(purrr)
library(stringr)

# Connect to postgres
source("W:\\RDA Team\\R\\credentials_source.R")
con <- connect_to_db("bold_vision")

# Pull in the response_id and race/ethnicity columns from the survey data
raw_svy_data <- dbGetQuery(con, "SELECT response_id, ar, \"as\", at, au, av, aw, ax, 
                       ay, az, ba, bb, bc, bd, be, bf, bg, bh, bi, bj, bk, bl, 
                       bm, bn, bo, bp, bq, br, bs FROM 
                       youth_thriving.raw_survey_data")

# isSouthAsian is a column in the data for South Asians
# isSoutheastAsian is a column in the data for Southeast Asians
# isCambodian is a column for Cambodians
svy_data <- raw_svy_data %>%
  mutate(isSouthAsian = 0,
         isSoutheastAsian = 0,
         isCambodian = 0)

# raceColumnList contains the race/ethnicity column names 
raceColumnList <- c("ar", "as", "at", "au", "av", "aw", "ax", "ay", "az", 
                    "ba", "bb", "bc", "bd", "be", "bf", "bg", "bh", "bi", 
                    "bj", "bk", "bl", "bm", "bn", "bo", "bp", "bq", "br", 
                    "bs","isSouthAsian","isSoutheastAsian","isCambodian")

svy_dd <- dbGetQuery(con, "SELECT variable, response_1 
                     FROM youth_thriving.bvys_datadictionary_2024")

# Filter svy_dd to contain the race/ethnicity column names, alongside with 
# the string description of race/ethnicity. It's helpful to view svy_dd to
# quickly decipher the meaning of each column during QA.
svy_dd <- svy_dd  %>%
  filter(variable %in% raceColumnList) 

```

Get unique responses of respondents who picked "Other" and provided a write-in.
```{r}
# unique_other_race_responses <- svy_data %>%
#   select(response_id, ba) %>%
#   filter(!is.na(ba)) %>%  # Optional: Filter out NA values if needed
#   mutate(ba_lower = tolower(ba)) %>%  # Convert ba to lowercase
#   distinct(ba_lower, .keep_all = TRUE) %>%  # Get unique ba values, keeping the first corresponding response_id
#   select(ba, response_id)  # Select original columns to keep
# 
# file_name <- paste0("unique_other_race_responses_", Sys.Date(),".csv") %>%
#   gsub("-", "", .)
# 
# write.csv(unique_other_race_responses, file = paste0("race recode/", file_name), row.names = FALSE)
```

Clean ba column
```{r}
svy_data_clean <- svy_data %>%
  mutate(ba_original = ba, # for QA purposes since ba will be overwritten/corrected later in script
         ba_clean = gsub("[Other race. Please write in:] ", "", ba, fixed=TRUE)) %>% # remove common piece of write-in string to simplify regex conditions later on
  mutate(ba_clean = tolower(ba_clean)) %>% # make all write-ins lowercase to help find distinct values
  mutate(ba_clean = gsub("and|\\/", ",", ba_clean)) %>% # replace "and" and "/" with "," to get a common delimiter 
  mutate(ba_clean = gsub("\\s*,\\s*",",", ba_clean)) %>% # clean white space for accurate regex conditions (e.g., ^ or $)
  rowwise() %>%
  mutate(ba_clean = list(as.list(strsplit(ba_clean, ",")[[1]]))) %>% # create a list of race/eth identities from write-in responses
  ungroup() 
```

Recode respondents who selected "Other Race" accordingly.
```{r}
# Define a function get_revised_response to recode the respondent's write-in 
# race(s) to racial categories already listed in the survey.
get_revised_response <- function(response) {
  response <- unlist(response)
  #If the respondent didn't mark "Other race", leave the function now.
  if (anyNA(response)) return('') 
  
  # Initialize an empty vector. This represents the respondent's racial composition. 
  races <- list()  
  if (grepl("^Native American$|^might be mixed with indigenous.$", response, ignore.case = TRUE)) 
  {races <- append(races, "ar")}
  
  if (grepl("^Mexican Indian$", response, ignore.case = TRUE)) # Recoding to Latinx and South Asian
  {races <- c(races, "au", "av","bc","isSouthAsian")}

  if (grepl("^black$|^i know i'm black$|^african american$|^afro$|^Carabian$|^Caribbean$|^Nigerian$|^Jamaican$", response, ignore.case = TRUE))
  {races <- append(races, "as")}

  if (grepl("^latina$|^latino$|^hispanic$|^hispanic salvadoran$|^Chicano$|^Mexican$|^Mexican American$|^Cuban$|^Puerto Rican$|^Purto Rican$|^Puertorrican$|^Bolivia$|^Nicaragua$", response, ignore.case = TRUE))
  {races <- append(races, "au")}

  if (grepl("^asain$", response, ignore.case = TRUE))
  {races <- append(races, "av")}
    
  if(grepl("^filipino$", response, ignore.case=TRUE)) 
    {races <- append(races, list("av", "bd"))}
  
  if(grepl("^Indian$", response, ignore.case=TRUE)) 
    {races <- append(races, list("av", "bc", "isSouthAsian"))}
  
  if(grepl("^South Asian$|^Sri Lankan$", response, ignore.case=TRUE)) 
    {races <- append(races, list("av", "isSouthAsian"))}
  
  if(grepl("^Japanese$", response, ignore.case=TRUE)) 
    {races <- append(races, list("av", "bg"))}

  if (grepl("^armenian$", response, ignore.case = TRUE))
  {races <- append(races, "ax")}

  if (grepl("^Cockasain$|^white$|^Russian$", response, ignore.case = TRUE))
  {races <- append(races, "ay")}

  if (grepl("^mixed white$", response, ignore.case = TRUE))
  {races <- append(races, list("ay", "ba"))}

  if (grepl("^null$|^mixed$|^multiracial$|^Belizean$|^two or more$|^i really don't know$", response, ignore.case = TRUE))
  {races <- append(races, "ba")}

  if (grepl("^belizean black$", response, ignore.case = TRUE))
  {races <- append(races, list("ba", "as"))}
  
  return(races)
}

# Extract the race of respondents who marked "Other race", 
# stored in the column revised_response.
svy_data_revised <- svy_data_clean %>%
  rowwise() %>%
  mutate(
    # Extract the revised response and correct ba if needed
    revised_response = list(unlist(map(ba_clean, get_revised_response))),
    # If revised_response doesn't contain ba, the respondent actually marked 
    # "Other race" incorrectly. Therefore, unmark "Other race" in the survey data.      
    ba = ifelse("ba" %in% revised_response, ba, NA)
    ) %>%
  ungroup() %>%
  mutate(  
    # Mark the respondent's correct race(s) based on revised_response
    across(all_of(setdiff(raceColumnList, "ba")),  
           ~ ifelse(grepl(cur_column(), revised_response), 1, .))
  ) 

```

Check and clean bh (Q4, Other Asian write-in response). If respondents didn't identify as Asian in Q3, force the response to Q4 to be NA.
```{r}
other_asian_responses <- svy_data_revised %>%
  filter(!is.na(bh)) # Optional: Filter out NA values if needed

# based on some of the distinct responses it seems like folks shouldn't have 
# been directed to the Q4. used the check below to confirm that although Asian (av)  
# was not selected in Q3, respondents have write-ins for Q4 (bh)
# 65 out of the 80 responses with write-ins selected Asian in Q3
qa_check <- other_asian_responses %>%
  select(response_id, av, aw, bh, bb:bg, bi, everything())

# clean bh column to remove folks where av = NA
svy_data_clean_bh <- svy_data_revised %>%
  mutate(bh_original = bh, # for QA purposes since ba will be overwritten/corrected later in script
         bh_clean = ifelse(av==1, bh, NA)) %>%
  # remove common piece of write-in string to simplify regex conditions later on
  mutate(bh_clean = gsub("[Other. Please write in:] ", "", bh_clean, fixed=TRUE))
```

Get unique responses of respondents who picked "Other Asian".
```{r}
unique_other_asian_responses <- svy_data_clean_bh %>%
  filter(!is.na(bh_clean))  %>% # Optional: Filter out NA values if needed
  select(response_id, bh_clean) %>%
  mutate(bh_lower = tolower(bh_clean)) %>%  # Convert bh to lowercase
  distinct(bh_lower, .keep_all = TRUE) %>%  # Get unique bh values, keeping the first corresponding response_id
  #select(bh_clean, response_id) %>%
  arrange(bh_clean)

# file_name <- paste0("unique_other_asian_responses_", Sys.Date(),".csv") %>%
#   gsub("-", "", .)
# 
# write.csv(unique_other_asian_responses, file = paste0("race recode/", file_name), row.names = FALSE)
```

Recode respondents who selected "Other Asian" accordingly.
```{r}
# Define a function get_revised_asian_response to recode the respondent's write-in 
# race(s) to racial categories already listed in the survey.
get_revised_asian_response <- function(response) {
  #If the respondent didn't mark "Other Asian", leave the function now.
  if (is.na(response)) return('') 
  
  # Initialize an empty vector. This represents the respondent's write-in response.
  ethnicities <- list()  
  
  if (grepl("^Bangladesh$|^Bangladeshi$|^Bengali$|^Pakistani$", response, ignore.case = TRUE))
  {ethnicities <- append(ethnicities, c("isSouthAsian"))}
  
  if (grepl("^Burmese$|^Indonesian$|^Laos$|^Thai$", response, ignore.case = TRUE))
  {ethnicities <- append(ethnicities, c("isSoutheastAsian"))}
  
  # We are choosing to NOT label Cambodians as "Other Asian" 
  if (grepl("^Cambodian$|^Khmer$", response, ignore.case = TRUE)) 
  {ethnicities <- append(ethnicities, c("isCambodian", "isSoutheastAsian","not_bh"))} 
  
  if (grepl("^Cambodian Chinese$", response, ignore.case = TRUE))
  {ethnicities <- append(ethnicities, c("bb", "isCambodian", "isSoutheastAsian","not_bh"))}
  
  if (grepl("^Cambodian/ Thai$", response, ignore.case = TRUE))
  {ethnicities <- append(ethnicities, c("isCambodian", "isSoutheastAsian"))}
  
  if (grepl("^Chinese Indonesian$", response, ignore.case = TRUE))
  {ethnicities <- append(ethnicities, c("bb", "isSoutheastAsian"))}
  
  if (grepl("^Filipino$", response, ignore.case = TRUE))
  {ethnicities <- append(ethnicities, c("bd", "isSoutheastAsian"))}
  
  if (grepl("^Southwest Asian$", response, ignore.case = TRUE))
  {ethnicities <- append(ethnicities, c("ax","not_bh"))}
  
  return(paste(ethnicities, collapse = "; "))
  
}

# Extract the race of respondents who marked "Other Asian", 
# stored in the column revised_asian_response.
svy_data_other_asian_recode <- svy_data_clean_bh %>%
  mutate(
    revised_asian_response = map_chr(bh_clean, get_revised_asian_response),
    
    # Mark the respondent's correct race(s) based on revised_asian_response. Adding \\b around cur_column ensures that we're looking for an exact string match
    across(all_of(setdiff(raceColumnList, c("bh"))),  
           ~ ifelse(str_detect(revised_asian_response, paste0("\\b", cur_column(), "\\b")), 1, .)),
    
    # If revised_asian_response contains "not_bh", the respondent actually marked 
    # "Other Asian" incorrectly. Therefore, unmark "Other race" in the survey data.  
    bh = ifelse(grepl("not_bh", revised_asian_response, ignore.case = TRUE), NA, bh),
  )


svy_data_other_asian_recode <- svy_data_other_asian_recode %>%
  # Respondents who already identified as Vietnamese or Filipino (without write-in)
  mutate(isSoutheastAsian = 
           case_when(
             bd == 1 ~ 1, # Filipino
             bf == 1 ~ 1, # Vietnamese
             isCambodian == 1 ~ 1, # Cambodian
             .default = isSoutheastAsian),
         # Respondents who identify as Asian Indian will also be labelled as South Asian
         isSouthAsian = 
           case_when(
             bc == 1 ~ 1, # Asian Indian,
             .default = isSouthAsian))

```

Check and clean br (NHPI/Other write-in responses for Survey Question 5)
```{r}
other_nhpi_responses <- svy_data_other_asian_recode %>%
  filter(!is.na(br)) # Optional: Filter out NA values if needed

# based on some of the distinct responses it seems like folks shouldn't have 
# been directed to Q5. used the check below to confirm that although NHPI (aw)  
# was not selected in Q3, respondents have write-ins for Q5 (br)
# only 12 of 46 responses with write-ins selected NHPI in Q3
qa_check <- other_nhpi_responses %>%
  select(response_id, aw, br, bj:bq, bs, everything())

# clean bh column to remove folks where av = NA
svy_data_clean_br <- svy_data_other_asian_recode %>%
  mutate(br_original = br, # for QA purposes since ba will be overwritten/corrected later in script
         br_clean = ifelse(aw==1, br, NA)) %>%
  # remove common piece of write-in string to simplify regex conditions later on
  mutate(br_clean = gsub("[Other. Please write in:] ", "", 
                                  br_clean, fixed=TRUE))
```

Get unique responses of respondents who picked "Other NHPI".
```{r}
# unique_other_nhpi_responses <- svy_data_clean_br %>%
#   select(response_id, br_clean) %>%
#   filter(!is.na(br_clean)) %>%  # Optional: Filter out NA values if needed
#   mutate(br_lower = tolower(br_clean)) %>%  # Convert bh to lowercase
#   distinct(br_lower, .keep_all = TRUE) %>%  # Get unique bh values, keeping the first corresponding response_id
#   select(br_clean, response_id) %>% # Select original columns to keep
#   arrange(br_clean)
# 
# file_name <- paste0("unique_other_nhpi_responses_", Sys.Date(),".csv") %>%
#   gsub("-", "", .)
# 
# write.csv(unique_other_nhpi_responses, file = paste0("race recode/", file_name), row.names = FALSE)
```

Recode respondents who selected "Other NHPI" accordingly.
```{r}
# Define a function get_revised_nhpi_response to recode the respondent's write-in 
# race(s) to racial categories already listed in the survey.


get_revised_nhpi_response <- function(response) {
  #If the respondent didn't mark "Other NHPI", leave the function now.
  if (is.na(response)) return('') 
  
  # Initialize an empty vector. This represents the respondent's write-in response.
  ethnicities <- list() 
  
  if (grepl("^Filipino$", response, ignore.case = TRUE)) 
  {ethnicities <- append(ethnicities, c("bd", "isSoutheastAsian"))}
  
  if (grepl("^Onieda$|^Shoshone Native American$", response, ignore.case = TRUE)) 
  {ethnicities <- append(ethnicities, c("ar", "not_br", "not_aw"))}
  
  if (grepl("^Panamanian$", response, ignore.case = TRUE)) 
  {ethnicities <- append(ethnicities, c("au", "not_br", "not_aw"))}
  return(paste(ethnicities, collapse = "; "))
}

# Extract the race of respondents who marked "Other NHPI", 
# stored in the column revised_nhpi_response.
svy_data_other_nhpi_recode <- svy_data_clean_br %>%
  mutate(
    revised_nhpi_response = map_chr(br_clean, get_revised_nhpi_response),
    #print('revised_nhpi_response', revised_nhpi_response),
    # Mark the respondent's correct race(s) based on revised_nhpi_response
    across(all_of(setdiff(raceColumnList, c("br"))),  
           #Adding \\b around cur_column ensures that we're looking for an exact string match
           ~ ifelse(str_detect(revised_nhpi_response, paste0("\\b", cur_column(), "\\b")), 1, .)),
    # If revised_nhpi_response contains "not_br", the respondent actually marked 
    # "Other NHPI" incorrectly. Therefore, unmark "Other NHPI" in the survey data.  
    br = ifelse(grepl("not_br", revised_nhpi_response, ignore.case = TRUE), NA, br),
    
    # If revised_nhpi_response contains "not_aw", the respondent actually marked 
    # "NHPI" incorrectly. Therefore, unmark "NHPI" in the survey data.
    aw  = ifelse(grepl("not_aw", revised_nhpi_response, ignore.case = TRUE), NA, aw)
  )
print(svy_data_other_nhpi_recode)
```


Creating columns to represent the race of each respondent. 
```{r}
svy_data_final <- svy_data_other_nhpi_recode %>%
  mutate(
    race_aian = case_when(is.na(ar) ~ 0, ar == 1 ~ 1),
    race_black = case_when(is.na(as) ~ 0, as == 1 ~ 1),  
    race_indigenous = case_when(is.na(at) ~ 0, at == 1 ~ 1),
    race_latinx = case_when(is.na(au) ~ 0, au == 1 ~ 1),
    race_asian = case_when(is.na(av) ~ 0, av == 1 ~ 1),
    race_nhpi = case_when(is.na(aw) ~ 0, aw == 1 ~ 1),
    race_swana = case_when(is.na(ax) ~ 0, ax == 1 ~ 1),
    race_white = case_when(is.na(ay) ~ 0, ay == 1 ~ 1),
    race_dwta = case_when(is.na(az) ~ 0, az == 1 ~ 1),
    # If respondents pick "Other race", they write in a response.
    race_other = case_when(is.na(ba) ~ 0, ba != "" ~ 1),
  )
```

Creating the nh_race column in svy_data, which is "latinx" if the respondent is 
Latinx. If the respondent identified as 2 or more races, the column is "nh_twoormore." 
If the respondent identified as a single race, the column is "nh_[insert chosen race]".
```{r}
svy_data_final <- svy_data_final %>%
  rowwise() %>%
  mutate(
    race_aian_indigenous = ifelse((race_aian==1 | race_indigenous==1), 1, 0),
    race_sum = sum(c(race_aian_indigenous, race_black, race_asian, race_nhpi, race_swana, race_white, race_other)),
    nh_race = case_when(
      race_latinx == 1 ~ 'latinx',             # Prioritize Latinx
      race_sum > 1 ~ 'nh_twoormor',            # If more than one race is selected
      (race_aian_indigenous == 1 & race_sum == 1) ~ 'nh_aian',   # If American Indian/Alaska Native
      (race_black == 1 & race_sum == 1) ~ 'nh_black',            # If Black/African American
      (race_indigenous == 1 & race_sum == 1) ~ 'nh_aian',  # If Indigenous
      (race_asian == 1 & race_sum == 1) ~ 'nh_asian',            # If Asian
      (race_nhpi == 1 & race_sum == 1) ~ 'nh_nhpi',              # If Native Hawaiian/Pacific Islander
      (race_swana == 1 & race_sum == 1) ~ 'nh_swana',            # If SWANA (Southwest Asian/North African)
      (race_white == 1 & race_sum == 1) ~ 'nh_white',            # If White
      (race_dwta == 1) ~ 'do_not_wish',          # If Don't want to answer
      (race_other == 1 & race_sum == 1) ~ 'nh_other',            # If Other race
      .default = "NA" # Non-response
    )
  ) %>%
  ungroup() %>%
  select(-race_sum)  # Remove the helper column if not needed

```

Creating acs_race column in svy_data, which is "latino" if the respondent is 
Latinx. Single races are: nh_aian (includes aian and indigenous), nh_asian,
nh_black, nh_nhpi, nh_white (includes white and swana per census methods) and 
nh_other (includes other, don't wish to answer, and NA). 
If the respondent identified as 2 or more races, the label is "nh_twoormor."
Note: labels are meant to match values in youth_thriving.acs_pums_race_pop_15_24 table. 
```{r}
svy_data_final <- svy_data_final %>%
  rowwise() %>%
  mutate(
    acs_race = case_when(
      nh_race == 'latinx' ~ 'latino',         # Rename to match population weight table in pg
      nh_race == 'nh_aian' ~ 'nh_aian',       # Already includes aian and indig.
      nh_race == 'nh_asian' ~ 'nh_asian',
      nh_race == 'nh_black' ~ 'nh_black',
      nh_race == 'nh_nhpi' ~ 'nh_pacisl',     # Replace with value in youth_thriving.acs_pums_race_pop_15_24 table
      nh_race == 'nh_white' ~ 'nh_white',
      nh_race == 'nh_swana' ~ 'nh_white',     # Per census methods
      nh_race == 'nh_other' ~ 'nh_other',
      nh_race == 'do_not_wish' ~ 'nh_other',
      nh_race == 'NA' ~ 'nh_other',
      nh_race == 'nh_twoormor' ~ 'nh_twoormor'
    )
  ) %>%
  ungroup()

# Quick check on counts
table(svy_data_final$acs_race)

# latino     nh_aian    nh_asian    nh_black    nh_other   nh_pacisl nh_twoormor    nh_white 
#   1877          67         355         551         192          23         154         225 

```

Create the detailed_race column in svy_data, which lists the racial 
composition of each respondent. The column does not list the respondent's 
specific ethnic origins.
```{r}
svy_data_final <- svy_data_final %>%
  rowwise() %>%
  mutate(detailed_race = {
    races <- c()  # Initialize an empty vector
    if (race_aian == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "ar"])
    if (race_black == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "as"])
    if (race_indigenous == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "at"])
    if (race_latinx == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "au"])
    if (race_asian == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "av"])
    if (race_nhpi == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "aw"])
    if (race_swana == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "ax"])
    if (race_white == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "ay"])
    if (race_dwta == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "az"])
    if (race_other == 1) races <- c(races, svy_dd$response_1[svy_dd$variable == "ba"])
    # Combine the non-empty races into a single string
    paste(races, collapse = "; ")
  }) %>%
  ungroup()

```

Create the detailed_asian column in svy_data, which lists the Asian ethnic 
composition of each Asian/Asian American respondent.
```{r}
svy_data_final <- svy_data_final %>%
  rowwise() %>%
  mutate(detailed_asian = {
    ethnicities <- c()  # Initialize an empty vector
    if (race_asian == 1) {
      if (!is.na(bb) && bb == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bb"])
      if (!is.na(bc) && bc == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bc"])
      if (!is.na(bd) && bd == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bd"])
      if (!is.na(be) && be == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "be"]);
      if (!is.na(bf) && bf == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bf"]);
      if (!is.na(bg) && bg == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bg"]);
      if (!is.na(bh) && bh == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bh"]);
      if (!is.na(bi) && bi == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bi"]);
      if (!is.na(isCambodian) && isCambodian == 1) {ethnicities <- c(ethnicities, "Cambodian")}
    }
    # Combine the non-empty ethnicities into a single string
    paste(ethnicities, collapse = "; ")
  }) %>%
  ungroup()

# Clean up bh_clean in order to add other Asian ethnicities to detailed_asian
svy_data_final <- svy_data_final %>%
  mutate(
    # Only apply transformations if bh_clean is not NA
    bh_clean = ifelse(
      is.na(bh_clean), 
      bh_clean,  # Keep as NA if it is NA
      bh_clean %>%
        # Remove specific ethnicities from bh_clean (case insensitive)
        str_remove_all(regex("cambodian|khmer|filipino|chinese", ignore_case = TRUE)) %>%
        
        # Replace / with ; and ensure consistent spacing
        str_replace_all("/", "; ") %>%
        
        # Convert entire string to lowercase first
        str_to_lower() %>%
        
        # Capitalize the first letter of each word
        str_to_title() %>%
        
        # Replace "Bangladesh" with "Bangladeshi"
        str_replace_all(regex("\\bBangladesh\\b", ignore_case = TRUE), "Bangladeshi") %>%
        
        # Remove leading/trailing whitespace
        str_trim(side = "both") %>%
        
        # Remove any leading semicolon with optional whitespace
        str_remove("^;\\s*") %>%
        
        # Ensure no extra space around semicolons
        str_replace_all("(\\w+)\\s*;\\s*(\\w+)", "\\1; \\2")
    )
  ) %>%
  mutate(
    # Concatenate detailed_asian and bh_clean only if bh_clean is not NA or empty
    detailed_asian = ifelse(
      !is.na(bh_clean) & bh_clean != "",
      # Concatenate detailed_asian and bh_clean only if detailed_asian is not NA or empty. Set detailed_asian to bh_clean if detailed_asian is NA or empty
      ifelse(!is.na(detailed_asian) & detailed_asian != "", paste0(detailed_asian, "; ", bh_clean), bh_clean),  
      detailed_asian  # Keep original detailed_asian if bh_clean is NA or empty
    )
  ) %>%
  ungroup()
```

Create the detailed_nhpi column in svy_data, which lists the NHPI ethnic 
composition of each NHPI respondent.
```{r}
svy_data_final <- svy_data_final %>%
  rowwise() %>%
  mutate(detailed_nhpi = {
    ethnicities <- c()  # Initialize an empty vector
    if (race_nhpi == 1) {
      if (!is.na(bj) && bj == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bj"])
      if (!is.na(bk) && bk == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bk"])
      if (!is.na(bl) && bl == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bl"])
      if (!is.na(bm) && bm == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bm"]);
      if (!is.na(bn) && bn == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bn"]);
      if (!is.na(bo) && bo == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bo"]);
      if (!is.na(bp) && bp == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bp"]);
      if (!is.na(bq) && bq == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bq"]);
      if (!is.na(br) && br == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "br"]);
      if (!is.na(bs) && bs == 1) ethnicities <- c(ethnicities, svy_dd$response_1[svy_dd$variable == "bs"]);
    }
    # Combine the non-empty ethnicities into a single string
    paste(ethnicities, collapse = "; ")
  }) %>%
  ungroup()
```

Export final data frame as relational table.
```{r}
# Get relevant columns from svy_data to put into a table in the database
final_df<-svy_data_final%>%
  select(response_id, nh_race, acs_race, race_aian_indigenous, race_aian, race_black, race_indigenous, race_latinx, 
         race_asian, race_nhpi, race_swana, race_white, race_dwta, race_other, detailed_race,
           isSouthAsian, isSoutheastAsian, detailed_asian, isCambodian, detailed_nhpi)

# Write table with metadata
table_name <- "race_ethnicity_data"
schema <- "youth_thriving"
indicator <- "A person-level table specifying the race and ethnicity of each respondent "
source <- "See QA doc for details: W:/Project/OSI/Bold Vision/Youth Thriving Survey/Documentation/QA_race_ethnicity_data.docx
Script: W:/Project/OSI/Bold Vision/Youth Thriving Survey/R/recoding_race.Rmd"
table_comment <- paste0(indicator, source)
dbWriteTable(con, c(schema, table_name), final_df,
             overwrite = TRUE, row.names = FALSE)

# Comment on table and columns
column_names <- colnames(final_df) # Get column names
column_comments <- c(
  "A unique, numeric ID for each respondent",
  "A string for whether the respondent is latinx, nh_twoormor, nh_aian_indigenous, nh_asian, nh_nhpi, nh_swana, nh_white, do_not_wish, nh_other",
  "A string the uses acs race labels: latino, nh_aian (incl. indigenous), nh_asian, nh_black, nh_other (incl. do not wish, NA, other), nh_pacisl (nh_nhpi), nh_twoormor, nh_white (incl. nh_swana)",
  "0/1 flag for whether the respondent is AIAN or indigenous from Mexico, Central America, or South America",
  "0/1 flag for whether the respondent is American Indian or Alaskan Native",
  "0/1 flag for whether the respondent is Black",
  "0/1 flag for whether the respondent is indigenous from Mexico, Central America, or South America",
  "0/1 flag for whether the respondent is Latinx/Latino/a/Hispanic",
  "0/1 flag for whether the respondent is Asian or Asian American",
  "0/1 flag for whether the respondent is Native Hawaiian and Pacific Islander",
  "0/1 flag for whether the respondent is South West Asian and North African",
  "0/1 flag for whether the respondent is white",
  "0/1 flag for whether the respondent did not wish to answer what their race is",
  "0/1 flag for whether the respondent is of another race not previously mentioned",
  "A string that lists the racial composition of each respondent. The column does not list the specific ethnic origins of each respondent",
  "0/1 flag for whether the respondent is South Asian",
  "0/1 flag for whether the respondent is Southeast Asian",  
  "A string that lists the Asian ethnic composition of each Asian/Asian American respondent",
  "0/1 flag for whether the respondent is Cambodian", 
  "A string that lists the NHPI ethnic composition of each NHPI respondent"
)

add_table_comments <- function(con, schema, table_name, indicator, source, column_names, column_comments) {
  comments <- character()
  comments <- c(comments, paste0("
    COMMENT ON TABLE ", schema, ".", table_name, " IS '", table_comment, "';"))
  for (i in seq_along(column_names)) {
    comment <- paste0("
      COMMENT ON COLUMN ", schema, ".", table_name, '."', column_names[i], '" IS \'', column_comments[i], "';
      ")
    comments <- c(comments, comment)
  }
  sql_commands <- paste(comments, collapse = "")
  dbSendQuery(con, sql_commands)
}


add_table_comments(con, schema, table_name, indicator, source, column_names, column_comments)

dbDisconnect(con)
```
