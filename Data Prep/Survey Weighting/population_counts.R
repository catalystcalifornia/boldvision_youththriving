...
vars_age <- vars_subject %>%
  filter(label %in% c("Estimate!!Total!!Total population!!SELECTED AGE CATEGORIES!!15 to 17 years",
...
age_counts <- age_sex %>%
  subset(select=names(age_sex) %in% vars_age$name) %>% # subset variables to those we need matching df above
  mutate(geoid="06037",
         name="Los Angeles County",
         weighting_group="Age") %>% # add variables for description in database
  pivot_longer(!c(geoid, name, weighting_group), 
               names_to="variable", 
               values_to="count") %>% # pivot dataframe
  mutate(variable=case_when(variable=='s0101_c01_021e' ~ "15-17", # recode categories
                            variable=='s0101_c01_023e' ~ "18-24")) %>%
  group_by(geoid, name, weighting_group) %>%
  mutate(percent=count/sum(count)) %>% # add percentage
  ungroup()
...
# pull data from dataframe
sex_counts <- age_sex %>%
  subset(select=names(age_sex) %in% vars_age_sex$name) %>% # subset variables to those we need matching df above
  mutate(geoid="06037", 
         name="Los Angeles County", 
         weighting_group="Sex at birth") %>% # add variables for description in database
  pivot_longer(!c(geoid,name,weighting_group), 
               names_to="variable",
               values_to="count")%>% # pivot dataframe
...
spas <- spas %>%
  select(OBJECTID, SPA, SPA_NAME, LABEL)
...
missing <- age_sex_tract %>%
  filter(!geoid %in% lac_tracts$GEOID) %>%
  select(geoid, vars_age_sex$name) # select vars with age/sex counts
...
missing <- lac_tracts %>%
  filter(!GEOID %in% tract_spa_xwalk$GEOID)
# mapview(missing) Just the islands didn't merge to SPA 8, merge back later

# convert to df and reduce columns
tract_spa_xwalk <- tract_spa_xwalk %>%
  st_drop_geometry() %>%
  as.data.frame() %>% 
  select(GEOID, SPA, SPA_NAME, LABEL) %>%
  rename_with(tolower) # make all columns lowercase

# # check results through a map
# xwalk_check <- lac_tracts %>%
...
#     smoothFactor = 0.5,  fillColor = ~ pal(spa_name), color = ~ pal(spa_name)) %>%
#   addPolygons(data = spa_check, weight = 1, color = "black") %>%
#   addLegend(
...
# pull data from dataframe
spa_counts_step1 <- age_sex_tract %>%
  select(geoid, all_of(vars_age$name)) %>% # subset variables to those we need matching df above
...
# check result
df %>% 
  group_by(weighting_group) %>%
  summarise(pop=sum(count),total=sum(percent))
# looks good
...
# st_write(obj=cb_2022_06_tract_500k, dsn = rda_con, layer = c("geographies_ca", "cb_2022_06_tract_500k"),
