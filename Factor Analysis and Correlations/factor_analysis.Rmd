...
small_svy_data_backup<-small_svy_data
...
test_result <- run_cfa(positive_identity_and_self_worth_AND_strong_minds_model)
```
...
for (i in component_models) {
  model_a<-cfa(i, data=small_svy_data, ordered=TRUE, std.lv=TRUE)
  assign(paste0("model_",i),model_a)
 
}

for (i in subcomponent_models) {
  model_a<-cfa(i, data=small_svy_data, ordered=TRUE, std.lv=TRUE)
  assign(paste0("model_",i),model_a)
 
}

# compare output from QA models to output in all_results
summary(`model_component_caring_families_and_relationships =~ q10a + q12a + dk + dl + dn`,fit.measures=TRUE,ci=TRUE, standardized=TRUE)

summary(`model_component_vibrant_communities =~ do + dp + dq + dr + ds + dt + du + dv`,fit.measures=TRUE,ci=TRUE,standardized=TRUE)

summary(`model_subcomponent_positive_emotions =~ ck + cl + cm`,fit.measures=TRUE,ci=TRUE,standardized=TRUE)

summary(`model_subcomponent_psychological_distress =~ ct + cu + cv + cw + cx + cy`,fit.measures=TRUE,ci=TRUE,standardized=TRUE)

summary(`model_subcomponent_experiences_of_racism_and_discrimination =~ ef + eg + eh + ei + ej + ek + el + em + en`,fit.measures=TRUE,ci=TRUE,standardized=TRUE)

# review combined measure of sm and pisf
test_qa<-cfa(positive_identity_and_self_worth_AND_strong_minds_model, data=small_svy_data, ordered=TRUE, std.lv=TRUE)

summary(test_qa,fit.measures=TRUE,ci=TRUE,standardized=TRUE)
# none of model fit measures check out so agree not to proceed

# QA table in r studio
nrow(all_results) # same number of rows
mean(all_results$factor_loading) # average same
# checked a handful of rows ordering by factor loading scores
# checked number of columns

### Other QA code
# test just standard code to make sure it matches up
 qa_model<-cfa(component_caring_families_and_relationships_model, data=small_svy_data, std.lv=TRUE)
 
 summary(qa_model, fit.measures=TRUE, ci=TRUE, standardized=TRUE)
 
 qa_model_2<-cfa(component_caring_families_and_relationships_model, data=small_svy_data, mimic =c("MPlus"), std.lv=TRUE)
 
 summary(qa_model_2, fit.measures=TRUE, standardized=TRUE)
 
  summary(`model_subcomponent_psychological_distress =~ ct + cu + cv + cw + cx + cy`, fit.measures=TRUE, standardized=TRUE)

   summary(`model_subcomponent_psychological_distress =~ ct + cu + cv + cw + cx + cy`, ci=TRUE, fit.measures=TRUE, standardized=TRUE)

# test out extraction
   std_loadings<-inspect(`model_subcomponent_psychological_distress =~ ct + cu + cv + cw + cx + cy`,"std.all")$lambda
   std_df<-as.data.frame(std_loadings)
   
   test_df<-parameterEstimates(`model_subcomponent_psychological_distress =~ ct + cu + cv + cw + cx + cy`, se=TRUE,pvalue=TRUE,ci=TRUE,standardized=TRUE, level=0.95)
   
    qa_psyc_distress<-cfa(subcomponent_psychological_distress_model, data=small_svy_data, std.lv=TRUE, ordered=TRUE)
       summary( qa_psyc_distress, ci=TRUE, fit.measures=TRUE, standardized=TRUE)

    run_cfa(subcomponent_psychological_distress_model)
...
all_results <- as.data.frame(rbind(component_results,subcomponent_results,remaining_results,test_result))
