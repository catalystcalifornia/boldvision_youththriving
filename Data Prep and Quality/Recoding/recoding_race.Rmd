...
qa_revised_other <- svy_data_clean %>%
  ...
  left_join(
    select(svy_data_revised, response_id, raceColumnList, ba_clean, revised_response), 
    by=c("response_id")) %>%
...
svy_data_final <- svy_data_final %>%
  ...
  mutate(
    ...
    detailed_asian = ifelse(
      ...
      # Concatenate detailed_asian and bh_clean only if detailed_asian is not NA or empty. 
...
final_df <- svy_data_final %>%
  select(response_id, nh_race, acs_race, race_aian_indigenous, race_aian, 
