...
qa <- dbGetQuery(con, "SELECT *
                     FROM youth_thriving.bvys_datadictionary_2024")
qa<-qa%>%filter(response_domain=='Demographics' & variable_name %in% c('Race','Race Asian Subgroup','Race NHPI Subgroup'))%>%filter(variable!="race")
...
qa_asian<-svy_data_clean %>%
 select(response_id,av,bb, bc, bd, be, bf, bg, bh, bi)

qa_nhpi<-svy_data_clean %>%
 select(response_id,aw,bj, bk, bl, bm, bn, bo, bp, bq, br, bs)
...
get_revised_response <- function(response) {
  ...
  if (grepl("^i really don't know,i know i'm black and might be mixed with indigenous.$", response, ignore.case = TRUE)) 
  ...
  if (grepl("^afro/latina$|^black and hispanic$|^latino / black$|^puerto rican/jamaican$", response, ignore.case = TRUE)) 
  {races <- append(races, list("as", "au", "not_ba"))}
  
  if (grepl("^african american,mexican indian$", response, ignore.case = TRUE)) # Recoding to Black, Latinx and South Asian
  ...
  if (grepl("^chicano/mexican american$|^mexican american$|^hispanic$|^hispanic salvadoran$|^Mexican$|^Cuban$|^Puerto Rican$|^Purto Rican$|^Puertorrican$|^Bolivia$|^Nicaragua$", response, ignore.case = TRUE))
  ...
  if(grepl("^South Asian$|^Sri Lankan$", response, ignore.case=TRUE)) 
    {races <- append(races, list("av", "isSouthAsian", "not_ba"))} # question not this one, mark also bh
  ...
  if (grepl("^mixed$|^mixed race$|^multiracial$|^mixed/multiracial$|^Belizean$|^two or more$", response, ignore.case = TRUE))
...
get_revised_asian_response <- function(response) {
  ...
  if (grepl("^taiwanese$|^tartarian/mongolian$|^central asian / uzbek$|^kazakh$", response, ignore.case = TRUE))
  ...
  {ethnicities <- append(ethnicities, c("ax", "not_bh"))} # unmark asian here
...
qa_revised_other <- svy_data_clean %>%
  select(response_id, raceColumnList) %>%
  left_join(select(svy_data_revised, response_id, raceColumnList, ba_clean, revised_response), by=c("response_id")) %>%
...
qa_revised_other_asian <- svy_data_clean %>%
  select(response_id, raceColumnList) %>%
  left_join(select(svy_data_revised_other_asian, response_id, raceColumnList, 
...
qa_revised_other_nhpi <- svy_data_clean %>%
  select(response_id, raceColumnList) %>%
  left_join(select(svy_data_revised_other_nhpi, response_id, raceColumnList, 
...
```{r}
svy_data_final <- svy_data_revised_other_nhpi %>%
  mutate(
...
sum(svy_data_final$race_black, na.rm=TRUE) #663
...
svy_data_final <- svy_data_final %>%
  ...
  mutate(
    ...
    nh_race = case_when(
      ...
      (race_indigenous == 1 & race_sum == 1) ~ 'nh_aian',  # If Indigenous # don't think this line is needed?
...
qa_check_black <- svy_data_clean %>%
  filter(as == 1) %>%
  select(response_id, as, ba_original, ba_clean, bh_original, bh_clean, br_original, br_clean) %>%
  left_join(svy_data_final, by=c("response_id")) %>%
  select(response_id, as.x, as.y, ba_original.x, ba_clean.x, bh_original.x, bh_clean.x, br_original.x, br_clean.x, nh_race, acs_race, everything())
...
qa<-svy_data_final%>%select(response_id,ar:ba,nh_race,acs_race,detailed_race)
qa_asian<-svy_data_final%>%select(response_id,av,bb:bh,nh_race,acs_race,detailed_asian)
```
