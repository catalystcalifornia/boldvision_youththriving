...
get_revised_response <- function(response) {
  ...
  if (grepl("^Mexican Indian$", response, ignore.case = TRUE))
  {races <- append(races, "at")}
...
Check and clean bh (Asian/Other write-in responses for Survey Question 4)
```{r}
...
svy_data_clean_bh <- svy_data_revised %>%
  ...
  mutate(bh_clean = gsub("[Other. Please write in:] ", "", 
                                   svy_data_clean_bh$bh_clean, fixed=TRUE))
```
...
get_revised_asian_response <- function(response) {
  ...
  {ethnicities <- append(ethnicities, c("bb", "isCambodian", "isSoutheastAsian"))}
  ...
  {ethnicities <- append(ethnicities, c("ax"))}
...
svy_data_other_asian_recode <- svy_data_clean_bh %>%
  mutate(
    ...
    # If revised_asian_response contains "not_av", the respondent actually marked 
    # "Asian" incorrectly. Therefore, unmark "Asian" (av) in the survey data.  
    av = ifelse(grepl("not_av", revised_asian_response, ignore.case = TRUE), NA, av)
  )
...
svy_data_final <- svy_data_final %>%
  rowwise() %>%
  mutate(
    race_sum = sum(c(race_aian, race_black, race_indigenous, race_asian, race_nhpi, race_swana, race_white)),
    nh_race = case_when(
      race_latinx == 1 ~ 'latinx',             # Prioritize Latinx
      race_sum > 1 ~ 'nh_twoormor',            # If more than one race is selected
      (race_aian == 1 & race_sum == 1) ~ 'nh_aian',              # If American Indian/Alaska Native
      ...
      (race_dwta == 1 & race_sum == 1) ~ 'do_not_wish',          # If Don't want to answer
...
final_df<-svy_data_final%>%
  select(response_id, nh_race,  race_aian, race_black, race_indigenous, race_latinx, 
...
column_comments <- c(
  ...
  "A string for whether the respondent is latinx, nh_twoormor, nh_aian, nh_asian, nh_nhpi, nh_swana, nh_white, do_not_wish, nh_other",
...
```{r}
total_responses <- nrow(svy_data_final)
race_breakdown_pct <- svy_data_final %>%
  summarise(
...
race_breakdown_count <- svy_data_final %>%
  summarise(
...
# race_aian_count 117
# race_black_count 671
# race_indigenous_count 6
# race_latinx_count 1877
# race_asian_count 481
# race_nhpi_count 43
# race_swana_count 51
# race_white_count 372
# race_dwta_count 4
# race_other_count 10
```