...
# qa check models
qa_model_check<-factor_analysis%>%group_by(model_name)%>%summarise(count=n(), vars=toString(unique(variable)))
...
run_predictions(subcomponent_psychological_distress_model)
...
fit_reduced <- factor_analysis_reduced  %>% filter(cfi>=.90 & tli >=.95 & (rmsea<.08 | srmr <.07))

factor_analysis_original <- dbGetQuery(con, "SELECT * FROM youth_thriving.factor_analysis
                              WHERE total_vars>=3")

fit_original_reduced <- factor_analysis_original  %>% filter(cfi>=.90 & tli >=.95 & (rmsea<.08 | srmr <.07))

# no change in number fitting all 3 parameters, compare fit change
factor_analysis_reduced <- factor_analysis_reduced %>%
  mutate(cfi_met=ifelse(cfi>=.95, 1,0),
         ...
         parameters_met=cfi_met+tli_met+rmsea_met+srmr_met)
...
compare <- factor_analysis_reduced %>% distinct(model_name, cfi, tli, rmsea, srmr,parameters_met) %>%
  left_join(factor_analysis_original %>% distinct(model_name, cfi, tli, rmsea, srmr,parameters_met), by="model_name")
...
# qa predicted results
subcomponent_psychological_distress_model <- 'subcomponent_psychological_distress =~ cy + cu + cw + cv + ct + cx'
qa_predict_factor<-cfa(subcomponent_psychological_distress_model, small_svy_data, std.lv=TRUE, ordered=TRUE)
qa_predict<-lavPredict(qa_predict_factor, newdata=small_svy_data, append.data=TRUE)%>%as.data.frame()
sum(is.na(qa_predict$subcomponent_psychological_distress))
sum(is.na(predicted_results$subcomponent_psychological_distress)) # has NA's
3444-167 # difference is NA values makes sense
mean(qa_predict$subcomponent_psychological_distress) 
mean(predicted_results$subcomponent_psychological_distress,na.rm=TRUE) # average is the same

qa_predict_factor<-cfa(subcomponent_experiences_of_racism_and_discrimination_model, small_svy_data, std.lv=TRUE, ordered=TRUE)
qa_predict<-lavPredict(qa_predict_factor, small_svy_data, append.data=TRUE)%>%as.data.frame()
sum(is.na(qa_predict$subcomponent_experiences_of_racism_and_discrimination))
sum(is.na(predicted_results$subcomponent_experiences_of_racism_and_discrimination)) # has NA's, difference is missing values ommitted in my result
mean(qa_predict$subcomponent_experiences_of_racism_and_discrimination) 
mean(predicted_results$subcomponent_experiences_of_racism_and_discrimination,na.rm=TRUE) # average is the same
test<-inspect(qa_predict_factor, "case.idx")

# qa nulls
subset <- predicted_results %>%
  select(-c(subcomponent_experiences_of_racism_and_discrimination, component_vibrant_communities))

# 2569 with complete cases
sum(complete.cases(subset))

# check responses for 2 components/subcomponents
exp_racism_data <- dbGetQuery(con, "SELECT * FROM youth_thriving.tot_freq_racial_justice
                              WHERE variable_name='Experiences Of Racism And Discrimination'")

exp_racism <- exp_racism %>% filter(response %in% c("Does not apply to me","Don't wish to answer"))

vibrant_data <- dbGetQuery(con, "SELECT * FROM youth_thriving.tot_freq_vibrant_community")

vibrant <- vibrant %>% filter(response %in% c("Don't know","Don't wish to answer"))
# might need to switch to a straight sum or average score here for each, e.g., # of experiences reported, # of resources reported
