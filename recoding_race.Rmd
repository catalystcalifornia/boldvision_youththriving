...
svy_data_clean <- svy_data %>%
  mutate(ba_clean = gsub("[Other race. Please write in:] ", "", ba, fixed=TRUE)) %>% # remove common piece of write-in string to simplify regex conditions later on
  ...
  rowwise() %>%
  mutate(ba_clean = list(as.list(strsplit(ba_clean, ",")[[1]]))) %>%
  ungroup()
...
get_revised_response <- function(response) {
  ...
  races <- c()  
  if (grepl("Native American|indigenous", response, ignore.case = TRUE)) 
  {races <- c(races, "ar")}
  
  if (grepl("black|african american|afro|Carabian|Caribbean|Nigerian|Jamaican", response, ignore.case = TRUE)) 
    {races <- c(races, "as")}
  ...
  if (grepl("Cockasain|white|Russian", response, ignore.case = TRUE)) 
    {races <- c(races, "ay")}
  if (grepl("null|mixed|multi|Belizean", response, ignore.case = TRUE)) 
    {races <- c(races, "ba")}
