...
# dbWriteTable(con, c(schema, table_name), all_results, overwrite = FALSE, row.names = FALSE)
...
```{r, include=FALSE, echo=FALSE}
# Update 2/13/25: We are not going to use a summative score for subcomponent_experiences_of_racism_and_discrimination_model. Instead, we reduced the number of items in the model.

# There are too many NA values for the "Experiences of Racism and Discrimination" so we are not going to use predicted factor scores for this subcomponent. Instead, we are going to compute a summative score.
# sum_experiences <- small_svy_data %>%
#   mutate(subcomponent_experiences_of_racism_and_discrimination_sum = rowSums(select(., c(ef, eg, eh, ei, ej, ek, el, em, en)), na.rm = TRUE)) %>%
#   select(response_id,subcomponent_experiences_of_racism_and_discrimination_sum)

# Merge subcomponent_experiences_of_racism_and_discrimination_sum with predicted_results
# predicted_results <- predicted_results %>%
#   left_join(sum_experiences, by="response_id")
```
...
qa_filepath <- "See QA doc for details: W:/Project/OSI/Bold Vision/Youth Thriving Survey/Documentation/QA_item_reduction_component_imputation.docx"
table_comment <- paste0(indicator, source)
#dbWriteTable(con, c(schema, table_name), predicted_results, overwrite = TRUE, row.names = FALSE)
