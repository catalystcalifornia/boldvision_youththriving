...
corrplot(
  ...
  insig = 'blank',
  diag = FALSE,
  tl.col = c(dark_pink,dark_pink, dark_pink, pink,pink,pink,pink,pink, yellow,yellow, dark_green, light_blue, blue, blue, blue, light_green), # Apply the custom group colors to the text labels
...
### BIPOC by Subcomponents 

```{r}

dt_bipoc <- dt_race %>% select(response_id, race_black, race_aian, race_indigenous, race_latinx, race_asian, race_swana, race_nhpi, race_white) %>% 
  mutate(bipoc = ifelse(race_white == 1, 0, 1))%>%
  mutate(across(everything(), ~replace(., is.na(.), 0))) 


dt_join <- left_join(dt_avg_scores, dt_bipoc, by = "response_id") %>%
  na.omit() 
...
corrplot(
  ...
  insig = 'blank',
  diag = FALSE,
  tl.col = c("black", dark_pink,dark_pink, dark_pink, pink,pink,pink,pink,pink, yellow,yellow, dark_green, light_blue, blue, blue, blue, light_green), # Apply the custom group colors to the text labels
  addgrid.col = "black" # Add grid lines to emphasize group separation
)
