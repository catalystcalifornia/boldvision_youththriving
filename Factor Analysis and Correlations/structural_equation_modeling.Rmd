...
get_sem_results_se <- function(fitted_model){
  ...
  # select(c(lhs,rhs,std.all,std.lv,p_is_significant,pvalue,se,bic,aic,r_squared,fit_measures_met,cfi,tli,rmsea,srmr)) %>%
    select(c(lhs,rhs,std.all,p_is_significant,pvalue,se,bic,aic,r_squared)) %>%
  arrange(desc(abs(std.all))) # Rearrange the dataframe so that std.all values are in descending order
  return(sem_output)
}
...
model_1j.6 <- '
  ...
  subcomponent_psychological_distress ~ component_caring_families_and_relationships +
    ...
    cisgender_mf
'
# Run model on unweighted data WITH FIML
...
# with standard errors
sem_output_1j.6_se <- get_sem_results_se(fit_1j.6) %>% 
  mutate(model_name="1j.6") %>%
  select(model_name, everything())

# Run model on weighted data WITH FIML and robust standard errors
fit_1j.6_weighted_fiml<- sem(model_1j.6, data = cisgender_svy_data, sampling.weights = "weights_final", std.lv = TRUE, missing = "fiml.x", se="robust")

sem_output_1j.6_weighted_fiml <- get_sem_results_se(fit_1j.6_weighted_fiml) %>% 
  mutate(model_name="1j.6_weighted_fiml") %>%
  select(model_name, everything())

# Run model on weighted data WITHOUT FIML
fit_1j.6_weighted <- sem(model_1j.6, data = cisgender_svy_data, sampling.weights = "weights_final", std.lv = TRUE, se="robust")

sem_output_1j.6_weighted <- get_sem_results_se(fit_1j.6_weighted) %>% 
  mutate(model_name="1j.6_weighted") %>%
  select(model_name, everything())

compare_1j.6_weighted <- compare_models_weighted(sem_output_1j.6_se, sem_output_1j.6_weighted_fiml,sem_output_1j.6_weighted)
...
# with standard errors
sem_output_2f.8_se <- get_sem_results_se(fit_2f.8) %>% 
  mutate(model_name="2f.8") %>%
  select(model_name, everything())

# Run model on weighted data WITH FIML and robust standard errors
fit_2f.8_weighted_fiml<- sem(model_2f.8, data = cisgender_svy_data, sampling.weights = "weights_final", std.lv = TRUE, missing = "fiml.x", se="robust")

sem_output_2f.8_weighted_fiml <- get_sem_results_se(fit_2f.8_weighted_fiml) %>% 
  mutate(model_name="2f.8_weighted_fiml") %>%
  select(model_name, everything())

# Run model on weighted data WITHOUT FIML
fit_2f.8_weighted <- sem(model_2f.8, data = cisgender_svy_data, sampling.weights = "weights_final", std.lv = TRUE, se="robust")

sem_output_2f.8_weighted <- get_sem_results_se(fit_2f.8_weighted) %>% 
  mutate(model_name="2f.8_weighted") %>%
  select(model_name, everything())

compare_2f.8_weighted <- compare_models_weighted(sem_output_2f.8_se, sem_output_2f.8_weighted_fiml,sem_output_2f.8_weighted)
...
# Create table of weighted, FIML final model results for all respondents and only cisgender respondents
```{r}
all_results_weighted <- as.data.frame(rbind(sem_output_1j.5_weighted_fiml, sem_output_1j.6_weighted_fiml, sem_output_2f.6_weighted_fiml, sem_output_2f.8_weighted_fiml))

# Write table with metadata
table_name <- "factor_analysis_sem_results_methodology"
schema <- "youth_thriving"
indicator <- "A table with the results of the structural equation modeling for the final models 1j.5, 1j.6, 2f.6, and 2f.8. These results are used to create regression tables in the Report Methodology. All models are run with weights, full information maximum likelihood (FIML), and robust standard errors. For each model, the independent variables are ordered from greatest to least effect size, with the absolute value taken. Models 1j.5 and 2f.6 were ran on all respondents, while models 1j.6 and 2f.8 were only ran on cisgender respondents."
source <- "Script: W:/Project/OSI/Bold Vision/Youth Thriving Survey/GitHub/AV/boldvision_youththriving/Factor Analysis and Correlations/structural_equation_modeling.Rmd "
qa_filepath <- "See QA doc for details: W:/Project/OSI/Bold Vision/Youth Thriving Survey/Documentation/QA_structural_equation_modeling_3.docx"
table_comment <- paste0(indicator, source)
# dbWriteTable(con, Id(schema, table_name), all_results_weighted, overwrite = TRUE, row.names = FALSE)
...
column_comments <- c(
  ...
  "The independent variable name",  
  "The independent variable effect size",
  "True/False on whether the effect is statistically significant at the p<0.05 level",
  "The p value",
  "The standard error",
  "Model fit measure BIC",
  "Model fit measure AIC",
  "Model fit measure R-squared"
)
