...
source("..\\Descriptive and Demographic Analyses\\crosstab_svyquestions.R")
```
...
get_grouped_bar_chart <- function(crosstab_df,title_text,string_question_var1,string_question_var2,svy_dd)
{
    ...
    # Set the chart title
    title_text <- paste(str_wrap(title_text, whitespace_only = TRUE, width = 50), collapse = "\n")

    # Set the x-axis title
    string_question_var1 <- str_wrap(string_question_var1, width = 80)
    ...
    x_labels <- str_wrap(unique(crosstab_df$response_var1), width = 15)
    ...
    subheading_theme <- element_text(hjust = 0.5, size = 11, colour = "black", family = font_axis_label, face = "plain",
                                     margin = margin(t = 5))
...
caption_theme <- element_text(hjust = 0, size = 11, colour = "black", family = font_caption, face = "plain", margin = margin(t = 10, l = 0))
# subheading_theme <- element_text(hjust = 0, size = 9, colour = "black", family = font_axis_label, face = "bold", margin = margin(t = 10, l = 0))
    ...
    caption_text <- "\nData Source: Catalyst California's calculations of Bold Vision Youth Thriving Survey, \n2024. For more information, see the 2025 Bold Vision Youth Thriving Report Methodology."
    # caption_text <- str_wrap(paste0(
    #   "Data Source: Catalyst California's calculations of Bold Vision Youth Thriving Survey, 2024. ",
    #   "For more information on the methodology, please refer to the 2025 Bold Vision Youth Thriving report."
    # ), width = 100)  # Apply wrapping only once

    num_categories <- length(unique(crosstab_df$response_var2))  # get number of categories for filling dependent on that later
    ...
    final_visual <- ggplot(crosstab_df, aes(fill=response_var2, y=rate, x=response_var1)) + 
      ...
      scale_fill_manual(values = if (num_categories == 5) diverging_bv_gradient else four_diverging) + #setting bar colors
      # Style legend title
      ...
      labs(title = title_text,
          fill = paste(
        string_question_var2,
        sep = "\n"
      ),
           # subtitle=string_question_var2, 
           caption = paste(
        caption_text,
        sep = "\n"
      )) +          # Set the chart title and caption
      theme_minimal() +
      theme(
        legend.position = "right",
        legend.margin = margin(0, 0, -5, 0),
        legend.box.margin = margin(-8,0,-8,0), # adjusting margin to reduce white space
        legend.text.align = 0,
...
title_text <- 'When youth have a "spark" in life, they are less likely to feel worthless'
...
title_text <- "When youth are told hurtful comments about their race, they are more likely to feel worthless"

# string_question_var2 <- "How often during the past 30 days did you feel worthless?"
string_question_var2 <- "During the past 30 days, \nI felt worthless..."
...
title_text <- "When youth lack access to quality health services, they are more likely to feel worthless"
string_question_var2 <- "During the past 30 days, \nI felt worthless..."
string_question_var1 <- "Poor quality health services get in the way of me living my best life."
...
title_text <- "When youth have someone who will be there for them, they have more hope for the future"
string_question_var2 <- "I feel hopeful when \nI think about my future."
