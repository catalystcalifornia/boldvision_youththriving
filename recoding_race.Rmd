...
Latinx. If the respondent identified as 2 or more races, the column is "nh_twoormore." If the respondent identified as a single race, the column is "nh_[insert chosen race]".
```{r}
svy_data <- svy_data %>%
  rowwise() %>%
  mutate(
    ...
    nh_race = case_when(
      ...
      race_aian == 1 ~ 'nh_aian',              # If American Indian/Alaska Native
      race_black == 1 ~ 'nh_black',            # If Black/African American
      race_indigenous == 1 ~ 'nh_indigenous',  # If Indigenous
      race_asian == 1 ~ 'nh_asian',            # If Asian
      race_nhpi == 1 ~ 'nh_nhpi',              # If Native Hawaiian/Pacific Islander
      race_swana == 1 ~ 'nh_swana',            # If SWANA (Southwest Asian/North African)
      race_white == 1 ~ 'nh_white',            # If White
      race_dwta == 1 ~ 'do_not_wish',          # If Don't want to answer
      race_other == 1 ~ 'nh_other',            # If Other race
    )
  ) %>%
  ungroup() %>%
...
column_comments <- c(
  ...
  "A string for whether the respondent is Latinx, two or more races, or a specific singlular race (e.g. Black, Asian)",
