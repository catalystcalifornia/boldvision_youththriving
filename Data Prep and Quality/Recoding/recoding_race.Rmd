...
get_revised_nhpi_response <- function(response) {
  ...
  if (grepl("^Polynesian$|^Pohnpeian$|^K<U+2206>ÍvÖnaka$", response, ignore.case = TRUE)) 
...
```{r}
asianDetailedList <- c("bb", "bc", "bd", "be", "bf", "bg", "bh", "bi", "isCambodian")
...
svy_data_detailed_asian <- svy_data_detailed_asian %>%
  ...
  mutate(
    detailed_asian_new = case_when(
      # Concatenate detailed_asian and bh_clean only if both are not NA or empty
      ((!is.na(bh_clean) & bh_clean != "") & (!is.na(detailed_asian) & detailed_asian != "")) ~ paste0(str_remove(detailed_asian, "^Other\\s*"), "; ", bh_clean),
      # Set detailed_asian to bh_clean if detailed_asian is NA or empty
      ((is.na(detailed_asian) | detailed_asian == "") & (!is.na(bh_clean) & bh_clean != "")) ~ bh_clean,
      # Otherwise (bh_clean is empty or NA), keep detailed_asian as is
      .default = detailed_asian)) %>%
...
qa_av <- svy_data_detailed_asian %>%
  select(response_id, bh_clean, detailed_asian, detailed_asian_new)
...
```{r}
nhpiDetailedList <- c("bj", "bk", "bl", "bm", "bn", "bo", "bp", "bq", "br", "bs")
...
column_comments <- c(
  ...
  "0/1 flag for whether the respondent is Cambodian (CC-Added Asian identity, not include in Other (Asian identity)",
  ...
  "The original response by the respondent if they included another racial identity",
  "The cleaned response used to recode identities and create detailed_race column",
  "The original response by the respondent if they included another Asian identity",
  "The cleaned response used to recode identities and create detailed_asian column",
  "The original response by the respondent if they included another NHPI identity",
  "The cleaned response used to recode identities and create detailed_nhpi column"
)
