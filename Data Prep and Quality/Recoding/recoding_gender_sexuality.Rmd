...
get_revised_sexuality_response <- function(response) {
  ...
  {sexualities <- append(sexualities, c("not_fx","fu"))} # I am not sure or questioning
...
## Check for bots and "odd" respondents using detailed gender and detailed sexuality.
```{r}
svy_data <- svy_data %>%
  rowwise() %>%
  mutate(detailed_gender = {
    genders <- c()  # Initialize an empty vector
    if (!is.na(fc) && fc == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "fc"])}
    if (!is.na(fd) && fd == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "fd"])}
    if (!is.na(fe) && fe == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "fe"])}
    if (!is.na(ff) && ff == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "ff"])}
    if (!is.na(fg) && fg == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "fg"])}
    if (!is.na(fh) && fh == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "fh"])}
    if (!is.na(fi) && fi == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "fi"])}
    if (!is.na(fj) && fj == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "fj"])}
    if (!is.na(fk) && fk == 1) {genders <- c(genders, svy_dd$response_1[svy_dd$variable == "fk"])}
    if (!is.na(fl)) {genders <- c(genders, paste(svy_dd$response_1[svy_dd$variable == "fl"], fl_clean))}
    if (!is.na(q22) && q22 == 1) {genders <- c(genders, paste('Sex:', svy_dd$response_1[svy_dd$variable == "q22"]))}    
    # Combine the non-empty genders into a single string
    paste(genders, collapse = "; ")
  }) %>%
  ungroup()

sort(unique(svy_data$detailed_gender))
```

```{r}
svy_data <- svy_data %>%
  rowwise() %>%
  mutate(detailed_sexuality = {
    sexualities <- c()  # Initialize an empty vector
    if (!is.na(fo) && fo == 1) {sexualities <- c(sexualities, svy_dd$response_1[svy_dd$variable == "fo"])}
    if (!is.na(fp) && fp == 1) {sexualities <- c(sexualities, svy_dd$response_1[svy_dd$variable == "fp"])}
    if (!is.na(fq) && fq == 1) {sexualities <- c(sexualities, svy_dd$response_1[svy_dd$variable == "fq"])}
    if (!is.na(fr) && fr == 1) {sexualities <- c(sexualities, svy_dd$response_1[svy_dd$variable == "fr"])}
    if (!is.na(fs) && fs == 1) {sexualities <- c(sexualities, svy_dd$response_1[svy_dd$variable == "fs"])}
    if (!is.na(ft) && ft == 1) {sexualities <- c(sexualities, svy_dd$response_1[svy_dd$variable == "ft"])}
    if (!is.na(fu) && fu == 1) {sexualities <- c(sexualities, svy_dd$response_1[svy_dd$variable == "fu"])}
    if (!is.na(fv) && fv == 1) {sexualities <- c(sexualities, svy_dd$response_1[svy_dd$variable == "fv"])}
    if (!is.na(fw) && fw == 1) {sexualities <- c(sexualities, paste(svy_dd$response_1[svy_dd$variable == "fw"]))}
    if (!is.na(fx)) {sexualities <- c(sexualities, paste(svy_dd$response_1[svy_dd$variable == "fx"], fx_clean))}
    # Combine the non-empty sexualities into a single string
    paste(sexualities, collapse = "; ")
  }) %>%
  ungroup() 

sort(unique(svy_data$detailed_sexuality))
```
...
column_comments <- c(
  ...
  "0/1: Cisgender (0)  vs TGNC (transgender, gender non-conforming) (1)",
