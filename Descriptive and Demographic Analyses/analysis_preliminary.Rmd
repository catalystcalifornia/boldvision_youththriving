...
The following summarizes how youth overall responded to each item in the Bold Vision Youth Thriving Survey by every component and subcomponent measured. The distribution of responses to each question within each subcomponent (e.g., Positive Emotions) is visualized using either a heatmap or table. The percentages shown are based on weighted estimates that adjust the youth thriving sample by age, sex, race, and Service Planning Area. Estimates that have a coefficient of variation greater than 40% are regarded as unstable and are suppressed from the heatmaps. 
