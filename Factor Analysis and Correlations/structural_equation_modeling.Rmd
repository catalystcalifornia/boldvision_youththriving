...
### Model 1j.7: FINAL; subsetted for black respondents only
...
# Run model on weighted data WITH FIML and robust standard errors
fit_1j.7_weighted_fiml_black<- sem(model_1j.7_black, data = svy_data_black, sampling.weights = "weights_final", std.lv = TRUE, missing = "fiml.x", se="robust")

sem_output_1j.7_weighted_fiml_black_test_2 <- get_sem_results_se(fit_1j.7_weighted_fiml_black) %>% 
  mutate(model_name="1j.7_weighted_fiml_black") %>%
  select(model_name, everything())
