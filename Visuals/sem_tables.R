...
# add special fonts
font_add(family = "Manifold Bold", regular = "W:/Project/OSI/Bold Vision/BV 2021/Deliverables/Bold Vision Fonts/Manifold/Fonts/manifoldcf-bold.otf")
font_add(family = "Manifold Extra Bold", regular = "W:/Project/OSI/Bold Vision/BV 2021/Deliverables/Bold Vision Fonts/Manifold/Fonts/manifoldcf-extrabold.otf")
font_icon <- "Manifold Bold"
font_icon_heavy <- "Manifold Extra Bold"
...
df <- df %>%
  mutate(std=ifelse(rhs_label %in% reverse_c | rhs_label %in% reverse_d, std.all*-1, std.all), # reverse
    ...
    mutate(
      y_pos = as.numeric(factor(rhs_label, levels = rev(unique(rhs_label)))) * 2.5  # multiply by 2 to have double spacing in the table
    )

df <- df %>% select(rhs_label, std, icon, direction, y_pos) # reduce columns

  line_positions <- seq(min(df$y_pos) - 1, max(df$y_pos) + 2.5, by = 2.5) # line positions with the double spacing
  
  header_y <- max(df$y_pos) + 2.5  # push it above the top row # add header row
  
table <- ggplot(df, aes(y = y_pos)) + # rows with double spacing
  geom_text(aes(x=.95,label=rhs_label),hjust=0,size=4.2,family= font_axis_label,colour = "black") + # coefficient label
  geom_text(aes(x = 0.85, label = icon), # outline color for arrow
            color = "black",  
            size = 8 + 0.2,  # slightly bigger than top layer
            family=font_icon_heavy
           ) +
  geom_text(aes(x=.85,label = icon, color = abs(std)), size=8, family=font_icon) + # arrow and then color based on absolute value
  geom_hline(yintercept = line_positions, color = gray, size = 0.1) + # lines between rows
  scale_color_gradient2(low = "#FCAEDC", high = "#F75EC1") + # color gradient for as coefficient goes from weaker to stronger effect (based on absolute value)
  ...
  labs(
    ...
    subtitle = "Ordered from <span style ='color: #F75EC1;'>largest</span> to <span style ='color: #FCAEDC;'>smallest </span>effect",
  ...
  annotate("text", x = 0.85, y = header_y, label = "Effect", hjust = 0.4, 
           family = font_axis_label, size = 4.2, fontface = "bold") +
...
table <- ggplot(df, aes(y = y_pos)) + # rows with double spacing
  ...
  geom_text(aes(x=.85,label = icon, color = abs(std)), size=8, family=font_icon) + # arrow and then color based on absolute value
  geom_hline(yintercept = line_positions, color = gray, size = 0.1) + # lines between rows
  scale_color_gradient2(low = "#F9A3AF", high = "#EF4A66") + # color gradient for as coefficient goes from weaker to stronger effect (based on absolute value)
  scale_x_continuous(limits = c(0.8, 1.5), expand = c(0, 0), breaks = NULL) + # x limits
  scale_y_continuous(
    breaks = df$y_pos,
    labels = df$rhs_label,
