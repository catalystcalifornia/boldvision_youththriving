...
for (q_num in grouped_response_types$question_number) 
{
  ...
  if (q_num %in% c('4', '5', '7', '10a','10b','12a', '24a')) 
  {
    ...
    nonresponse_by_respondent_df[[new_col_name]] <- ifelse(
      ...
      (q_num == '7' & (svy_data$bu == 1 | svy_data$by == 1)) | # The respondent is a full-time or part-time student, tf
      (q_num == '10a' & svy_data$q10 == 1) |  # The respondent has sparks in their life, mc
...
num_questions = ncol(nonresponse_by_respondent_df) - 1
  
nonresp_by_respondent <- nonresponse_by_respondent_df %>%
  mutate(
      nonresponse_count = rowSums(select(., -response_id), na.rm = TRUE), # na.rm=TRUE skips the NA values to calculate the sums
      nonresponse_rate = round((nonresponse_count / num_questions) * 100, 1),
      response_count = num_questions - nonresponse_count,
      response_rate = 100 - nonresponse_rate)  %>%
   relocate(c(nonresponse_count, nonresponse_rate, response_count, response_rate), .after = response_id)
```

# For each question, calculate the counts and rates of nonresponse and response
```{r}
# Calculate the nonresponse_flag_qnums' column sums excluding response_id and convert to data frame
nonresp_by_question <- as.data.frame(colSums(select(nonresponse_by_respondent_df, -response_id), na.rm = TRUE))
# Rename the column to 'nonresponse_counts'
...
non_na_counts <- as.data.frame(colSums(!is.na(select(nonresponse_by_respondent_df, -response_id))))
...
nonresp_by_question <- nonresp_by_question %>%
  ...
  mutate(
    nonresponse_rate = round(nonresponse_counts / non_na_counts * 100, 1),  # Use non_na_counts as denominator
...
# Export nonresp_by_respondent dataframe as a table with comments
```{r}
# Write table nonresp_by_respondent with metadata
table_name <- "nonresp_by_respondent"
...
dbWriteTable(con, c(schema, table_name), nonresp_by_respondent,
...
column_names <- colnames(nonresp_by_respondent) # Get column names
column_comments <- c(
...
table_name <- "nonresp_by_question"
schema <- "youth_thriving"
