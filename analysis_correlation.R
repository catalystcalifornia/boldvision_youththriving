...
true_factors_test <-c("Never true","Sometimes true","Often true","Always true") 

df_final <- df_almost_final %>% 
  mutate(response = factor(response, levels = true_factors_test)) 

##next step is updating bv data dictionary and then selecting only columns needed and averaging by subcomponent and then component
