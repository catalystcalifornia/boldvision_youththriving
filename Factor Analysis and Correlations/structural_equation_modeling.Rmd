...
```
```{r}
results_weighted <- as.data.frame(rbind(sem_output_1j.5_weighted %>% filter(p_is_significant==TRUE), sem_output_2f.6_weighted %>% filter(p_is_significant==TRUE)))

counting <- results_weighted %>%
  group_by(rhs) %>%
  count() %>%
  arrange(desc(n))
```
