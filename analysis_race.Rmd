...
#EXAMPLE C
df_chi_test <- wtd.chi.sq(svy_data$race, svy_data$cm, weight=svy_data$weights_final, na.rm=TRUE,
drop.missing.levels=FALSE) %>% as.data.frame() %>% t()
#NEED TO CLEAN UP AND NAME THIS data frame columns and row and then move on to making funciton.   


  setNames(cbind(rownames(df_chi_test), df_chi_test, row.names = "cm"), 
         c("variable", "chisq_value", "degrees_of_freedom", "p_value"))


rownames(df_chi_test) <- NULL
colnames(df_chi_test) <- c("variable", "chisq_value", "degrees_of_freedom", "p_value")

#create an empty table and add the rows to it to eventually have one table and then merge that table with data dictionary to know what question/questionnumber/subquestion it is associated with. 
# 
# #EXAMPLE D
