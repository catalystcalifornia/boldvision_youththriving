...
```{r}
response = "rican"
grepl("latina|latino|hispanic|Chicano|Mexican|Cuban|Puerto Rican|Purto Rican|Puertorrican|Bolivia|Nicaragua", response, ignore.case = TRUE)
```
...
get_revised_response <- function(response) {
  ...
  if (grepl("black|african american|afro|Carabian|Caribbean|Nigerian|Jamaican", response, ignore.case = TRUE)) 
    {races <- c(races, "as")}
  if (grepl("latina|latino|hispanic|Chicano|Mexican|Cuban|Puerto Rican|Purto Rican|Puertorrican|Bolivia|Nicaragua", response, ignore.case = TRUE)) 
    {races <- c(races, "au")}
  ...
  if (grepl("Cockasain|white|Russian", response, ignore.case = TRUE)) 
    {races <- c(races, "ay")}
  if (grepl("Two or more|null|mixed|multi|Belizean", response, ignore.case = TRUE)) 
    {races <- c(races, "ba")}
...
get_revised_asian_response <- function(response) {
  ...
  # if (grepl("Southwest|Uzbek|Kazakh", response, ignore.case = TRUE))
  ...
  if (grepl("Pakistani|Bangladesh|Bengali", response, ignore.case = TRUE))
  ...
  if (grepl("N/A|None|Doesn't apply|Non|Mexican|African|Hispanic|Pakistani|Southwest Asian|Hawaiian|Filipino|German", response, ignore.case = TRUE)) 
...
get_revised_nhpi_response <- function(response) {
  ...
  if (grepl("N/A|None|Doesn't apply|Non|not|na|Shoshone|Onieda", response, ignore.case = TRUE)) 
