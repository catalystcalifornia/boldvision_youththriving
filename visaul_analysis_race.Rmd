...
```{r, echo = FALSE}
#### NEXT CATEGORY 
df <- svy_dd%>%filter(response_domain=="Caring Families And Relationships" & variable_name=="Relationships/Support") # filter dictionary just for questions of interest
...
my_height <- 0.95 # height and width of ggplot
my_width <- 0.95
df$response <- factor(x=df$response, levels = count_factors) # factor levels to order plot by
# plot styling
visual<-ggplot(data = df, aes(x = response, y = race_labels,fill=rate))+
  geom_tile(height=my_height, width=my_width, color="white",lwd = 1.5,
            linetype = 1)+
...
```{r, echo = FALSE}
#### NEXT CATEGORY 
df <- svy_dd%>%filter(response_domain=="Positive Identity And Self-Worth" & variable_name=="Sparks") # filter dictionary just for questions of interest
