...
Clean up responses where survey skip logic failed (e.g., If respondents didn't identify as Asian in Q3, force the response to Q4 to be NA.; same for NHPI and Q5)
Also clean all write-in columns for race questions (ba, bh, br) and create an _original col for QA purposes
```{r}
...
# Clean up write-in columns
svy_data_clean <- svy_data_clean %>%
  mutate(ba_original = ba, 
         ...
         br_original = br, # for QA purposes since ba will be overwritten/corrected later in script
         br_clean = gsub("[Other. Please write in:] ", "", br, fixed=TRUE)) %>% # remove common piece of write-in string to simplify regex conditions later on
  
  mutate(ba_clean = tolower(ba_clean),
         bh_clean = tolower(bh_clean),
         br_clean = tolower(br_clean)) %>% # make all write-ins lowercase to help find distinct values
  mutate(ba_clean = gsub("and|\\/", ",", ba_clean)) %>% # replace "and" and "/" with "," to get a common delimiter (only needed for ba)
  mutate(ba_clean = gsub("\\s*,\\s*",",", ba_clean),
         bh_clean = gsub("\\s*,\\s*",",", bh_clean),
         br_clean = gsub("\\s*,\\s*",",", br_clean)) %>% # clean white space for accurate regex conditions (e.g., ^ or $)
  rowwise() %>%
...
qa_clean <- svy_data_clean %>%
  select(response_id, ba, ba_original, ba_clean, bh, bh_original, bh_clean, br, br_original, br_clean, everything())

```

Get unique responses of respondents who picked "Other", "Other Asian", and "Other NHPI" and provided a write-in response.
```{r}
...
Create functions to recode race responses accordingly
```{r}
...
svy_data_other_asian_recode <- svy_data_clean_bh %>%
  mutate(
    ...
    across(all_of(setdiff(raceColumnList, c("bh"))),  
           ~ ifelse(str_detect(revised_asian_response, paste0("^", cur_column(), "$")), 1, .)),
...
svy_data_other_nhpi_recode <- svy_data_clean_br %>%
  mutate(
    ...
    across(all_of(setdiff(raceColumnList, c("br"))),  
           ...
           ~ ifelse(str_detect(revised_nhpi_response, paste0("^", cur_column(), "$")), 1, .)),
...
sum(svy_data_final$race_black, na.rm=TRUE)
```
