...
compare_models_weighted <- function(unweighted_output, weighted_output)
{
  unweighted_sig_vars_ordered <- unweighted_output %>% 
    ...
    select(model_name,rhs,std.all.abs, std.all)
  ...
  return(rbind(unweighted_sig_vars_ordered, weighted_sig_vars_ordered))
}
```
...
# Run model on weighted data
fit_1j.5_weighted <- lavaan.survey(fit_1j.5, svy_data_weighted)

sem_output_1j.5_weighted <- get_sem_results(fit_1j.5_weighted) %>% 
  mutate(model_name="1j.5_weighted") %>%
  select(model_name, everything())

fit_test <- sem(model_1j.5, data = svy_data, sampling.weights = "weights_final", std.lv = TRUE, missing = "fiml.x")
summary(fit_test)
sem_output_1j.5_weighted_qa <- get_sem_results(fit_test) %>% 
  mutate(model_name="1j.5_weighted_qa") %>%
  select(model_name, everything())

fit_test_2 <- sem(model_1j.5, data = svy_data, sampling.weights = "weights_final", std.lv = TRUE)
sem_output_1j.5_weighted_qa_2 <- get_sem_results(fit_test_2) %>% 
  mutate(model_name="1j.5_weighted_qa_2") %>%
  select(model_name, everything())


compare_1j.5_weighted <- compare_models_weighted(sem_output_1j.5, sem_output_1j.5_weighted)
```
...
# Run model on weighted data
fit_2f.6_weighted <- lavaan.survey(fit_2f.6, svy_data_weighted)

sem_output_2f.6_weighted <- get_sem_results(fit_2f.6_weighted) %>% 
  mutate(model_name="2f.6_weighted") %>%
  select(model_name, everything())

compare_2f.6_weighted <- compare_models_weighted(sem_output_2f.6, sem_output_2f.6_weighted)
```
...
schema <- "youth_thriving"
indicator <- "A table with the results of the structural equation modeling for the unweighted and weighted final models 1j.5, 1j.6, 2f.2, 2f.6, and 2f.8. For each model, the independent variables are ordered from greatest to least effect size (with the absolute value taken). Only the statistically significant (p<0.05) independent variables are included in this table."
source <- "Script: W:/Project/OSI/Bold Vision/Youth Thriving Survey/GitHub/AV/boldvision_youththriving/Factor Analysis and Correlations/structural_equation_modeling.Rmd "
