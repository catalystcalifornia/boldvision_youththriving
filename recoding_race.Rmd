...
#   mutate(ba_clean = as.character(ba_clean)) %>%
#   filter(!is.na(ba)) %>%  # Optional: Filter out NA values if needed
#   distinct(ba, .keep_all = TRUE) %>%  # Get unique ba values, keeping the first corresponding response_id
#   select(ba, ba_clean, response_id)  # Select original columns to keep
# 
...
#   distinct(bh_clean, .keep_all = TRUE) %>%  # Get unique ba values, keeping the first corresponding response_id
#   select(bh, bh_clean, response_id)  # Select original columns to keep
# 
...
#   distinct(br_clean, .keep_all = TRUE) %>%  # Get unique ba values, keeping the first corresponding response_id
#   select(br, br_clean, response_id)  # Select original columns to keep
# 
...
# American Indian and Alaska Native -> AR
# Black or African American -> AS
# Indigenous from Mexico, Central America, or South America -> AT
# Latinx/Latino/a/Hispanic -> AU
# Asian or Asian American -> AV
# Native Hawaiian and Pacific Islander -> AW
# South West Asian and North African -> AX
# White -> AY
# Don't Wish to Answer -> AZ
# Other Race -> BA
...
get_revised_response <- function(response) {
  ...
  {races <- append(races, "au", "not_ba")}

  if (grepl("^asain$", response, ignore.case = TRUE))
  {races <- append(races, "av", "not_ba")}
  ...
  if(grepl("^South Asian$|^Sri Lankan$", response, ignore.case=TRUE)) 
    {races <- append(races, list("av", "isSouthAsian", "not_ba"))}
...
get_revised_nhpi_response <- function(response) {
  ...
  if (grepl("^Filipino$", response, ignore.case = TRUE)) 
  {ethnicities <- append(ethnicities, c("bd", "isSoutheastAsian", "br"))}
