...
# check
# table(small_svy_data$do,useNA='always')
# table(raw_svy_data$do,useNA='always')
```
...
demographics_binary_data <- dbGetQuery(con, "SELECT * FROM youth_thriving.demographics_binary_data")
...
# check
# colSums(race_ethnicity_acs)
# table(race_ethnicity_data$acs_race,useNA='always')
# filter(race_ethnicity_data, response_id=='136135266')
# filter(race_ethnicity_acs, response_id=='136135266')


poverty_rates <- dbGetQuery(con, "SELECT response_id, perc_below_100_fpl, perc_below_200_fpl FROM youth_thriving.poverty_rate_data")  
...
svy_data <- svy_data %>%
  ...
  filter(!is.na(org_sp_an)) %>%  # Remove NA values, 31 observations
...
svy_data <- svy_data %>%
  mutate(
      ...
      personal_safety = ifelse(q19 == 5, 1, 
      ...
      opportunities_for_community_involvement = ifelse(dm == 1, 0, 
...
table(svy_data$q19,useNA='always')
table(svy_data$dm,useNA='always')
table(svy_data$cr,useNA='always')
table(svy_data$cs,useNA='always')
sum(is.na(svy_data$cs))
sum(is.na(svy_data$cr))
raw_svy_data%>%filter(!is.na(org_sp_an))%>%count(q10)
table(svy_data$q10,useNA='always')
...
get_sem_results <- function(fitted_model){
  ...
  select(c(lhs,rhs,std.lv,p_is_significant,pvalue,bic,aic,r_squared,fit_measures_met,cfi,tli,rmsea,srmr)) %>%
  arrange(desc(std.lv)) # Rearrange the dataframe so that std.lv values are in descending order
  return(sem_output)
}
```
...
fitMeasures(fit_1, c("chisq", "df", "pvalue", "cfi", "rmsea","tli","srmr","rsquare"), fm.args = list(robust = FALSE))
lavInspect(fit_1, "rsquare")
...
### Model 1i: add spa; removing high_poverty_area and undocumented
...
### Model 1j: add high_poverty_area back in; remove spa
...
all_results <- as.data.frame(rbind(sem_output_1,sem_output_1a,sem_output_1b,sem_output_1c,sem_output_1d,sem_output_1e,sem_output_1f,sem_output_1g,sem_output_1h,sem_output_1i,sem_output_1j,sem_output_2,sem_output_2a,sem_output_2b,sem_output_2c,sem_output_2d,sem_output_2e,sem_output_2f))

# Write table with metadata
table_name <- "factor_analysis_sem_results"
