...
get_revised_gender_response <- function(response) {
  ...
  if (grepl("^indigiqueer$|^bigender$|^queer$|^gender expansive$", response, ignore.case = TRUE)) 
  ...
  {genders <- append(genders, c("fj","not_fl"))} # I do not have a gender/gender identity
...
get_revised_sexuality_response <- function(response) {
  ...
  if (grepl("^but i am open minded and respect every one$|^homophohe$|^i i'm rick grimes$|^i like dinosaurs$|^i⊙m not gay$|^null$|^unlabeled$", response, ignore.case = TRUE)) # Not Something Else
  {sexualities <- append(sexualities, c("not_fx"))} 
  
  if (grepl("^aroace$|^aromantic$|^aromantic as well$|^demisexual$|^fluid$|^have no interest in relationships with anyone$|^heteroromantic$|.*m never straight i just like what i like at that given time$|^none$|^omnisexual$|^pansexual$|^specifically demisexual/demi romantic$", response, ignore.case = TRUE)) # Something Else, another sexuality
