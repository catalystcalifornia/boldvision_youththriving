...
race_lgbtqia <- svy_data %>%
  ...
  select(response_id, nh_race, cishet_lgbtqia, cu, cy, weights_final) %>%
...
```{r}
# Get the count column for the crosstab
crosstab_count <- race_lgbtqia_data %>%
  group_by(race_lgbtqia, cy_binary) %>%
  summarise(count = n(), .groups = "drop")

# Get the weighted rates for the crosstab
crosstab_weighted <- race_lgbtqia_data %>% as_survey_design(weights = weights_final) %>%
  group_by(race_lgbtqia, cy_binary) %>%
  summarise(rate = survey_prop(vartype=c("cv","se"),level=.90)) %>% 
  mutate(rate = rate * 100,
         rate_se = rate_se * 100) %>%  # Convert to percentages
  ungroup()

# Crosstab table for race groups by lgbtqia+ and a REVISED binary psych distress question, cy
crosstab_race_lgbtqia_psych_worthless <- crosstab_count %>% 
  left_join(crosstab_weighted, by=c("race_lgbtqia", "cy_binary")) %>%
  filter(!(if_any(cy_binary,is.na)))
```
...
crosstab_weighted <- race_lgbtqia_data %>% as_survey_design(weights = weights_final) %>%
  group_by(race_lgbtqia, cu_binary) %>%
  summarise(rate = survey_prop(vartype=c("cv","se"),level=.90)) %>% # Automatically ensures sum = 100%
  mutate(rate = rate * 100,
         rate_se = rate_se * 100) %>%  # Convert to percentages
  ungroup()

# Crosstab table for race groups by lgbtqia+ and ANOTHER binary psych distress question, cu
crosstab_race_lgbtqia_psych_hopeless <- crosstab_count %>% 
  left_join(crosstab_weighted, by=c("race_lgbtqia", "cu_binary")) %>%
  filter(!(if_any(cu_binary,is.na)))
```
...
```{r}
# Get the count column for the crosstab
crosstab_count <- lgbtqia_sparks_data %>%
  group_by(lgbtqia_sparks, cu_binary) %>%
  summarise(count = n(), .groups = "drop") 

# Get the weighted rates for the crosstab
crosstab_weighted <- lgbtqia_sparks_data %>% as_survey_design(weights = weights_final) %>%
  group_by(lgbtqia_sparks, cu_binary) %>%
  summarise(rate = survey_prop(vartype=c("cv","se"),level=.90)) %>% # Automatically ensures sum = 100%
  mutate(rate = rate * 100,
         rate_se = rate_se * 100) %>%  # Convert to percentages
...
# Crosstab table for lgbtqia+ by whether they have spark and binary psych distress question, cu
crosstab_lgbtqia_sparks_psych_hopeless <- crosstab_count %>% 
  left_join(crosstab_weighted, by=c("lgbtqia_sparks", "cu_binary")) %>%
...
indicator <- "A crosstab table for race by lgbtqia+ identity and a REVISED binary psych distress question - did the respondent feel worthless in the past 30 days?"
...
column_comments <- c(
  "Shows the race and gender/sexuality identity",
  "Binary variable for psychological distress question cy, did the respondent feel worthless in the past 30 days?",  
  "The raw, unweighted count of the grouping (race, gender/sexuality, and feelings of worthlessnesss)",  
  "The rate of the race+gender/sexuality group that experienced feelings of worthlessness (or not)",
  "CV of the rate",
...
schema <- "youth_thriving"
indicator <- "A crosstab table for race by lgbtqia+ identity and a REVISED binary psych distress question - did the respondent feel hopeless in the past 30 days?"
source <- "Script: W:/Project/OSI/Bold Vision/Youth Thriving Survey/GitHub/AV/boldvision_youththriving/Descriptive and Demographic Analyses/crosstab_LGBTQIA_race.Rmd "
qa_filepath <- "See QA doc for details: W:/Project/OSI/Bold Vision/Youth Thriving Survey/Documentation/QA_crosstab_LGBTQIA_race.docx"
table_comment <- paste0(indicator, source)
...
column_comments <- c(
  "Shows the race and gender/sexuality identity",
  "Binary variable for psychological distress question cu, did the respondent feel hopeless in the past 30 days?",  
  "The raw, unweighted count of the grouping (race, gender/sexuality, and feelings of hopelessnesss)",  
  "The rate of the race+gender/sexuality group that experienced feelings of hopelessnesss (or not)",
  "CV of the rate",
  "Standard error of the rate",
  "Variable name of the psychological distress question",
  "The psychological distress survey question",
...
schema <- "youth_thriving"
indicator <- "A crosstab table lgbtqia+ identity by whether they have sparks and a REVISED binary psych distress question - did the respondent feel hopeless in the past 30 days?"
source <- "Script: W:/Project/OSI/Bold Vision/Youth Thriving Survey/GitHub/AV/boldvision_youththriving/Descriptive and Demographic Analyses/crosstab_LGBTQIA_race.Rmd "
qa_filepath <- "See QA doc for details: W:/Project/OSI/Bold Vision/Youth Thriving Survey/Documentation/QA_crosstab_LGBTQIA_race.docx"
table_comment <- paste0(indicator, source)
dbWriteTable(con, Id(schema, table_name), crosstab_lgbtqia_sparks_psych_hopeless, overwrite = TRUE, row.names = FALSE)
...
column_comments <- c(
  ...
  "The rate of the gender/sexuality+sparks group that experienced feelings of hopelessnesss (or not)",
  "CV of the rate",
  "Standard error of the rate",
  "Variable name of the psychological distress question",
  "The psychological distress survey question",
