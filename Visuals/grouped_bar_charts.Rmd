...
get_grouped_bar_chart <- function(crosstab_df,title_text,string_question_var1,string_question_var2,svy_dd)
{
    ...
    x_labels <- str_wrap(crosstab_df$response_var1, width = 20)
...
x_title_theme <- element_text(hjust = 0, size = 12, colour = "black", family = font_axis_label, face = "bold", margin = margin(t = 10, l = 0))
...
# subheading_theme <- element_text(hjust = 0, size = 9, colour = "black", family = font_axis_label, face = "bold", margin = margin(t = 10, l = 0))
    ...
    final_visual <- ggplot(crosstab_df, aes(fill=response_var2, y=rate, x=response_var1)) + 
      ...
      guides(fill=guide_legend(title=string_question_var2,   # Style legend title
    ...
    base_path <- paste0("W:/Project/OSI/Bold Vision/Youth Thriving Survey/Deliverables/", 
                        unique(crosstab_df$component_var2), "/", 
...
raw_crosstab <- cross_tab_df(svy_data,"q10","cy",svy_dd)
...
title_text <- 'Having a "spark" in life improves mental health'
string_question_var2 <- "How often during the past 30 days did you feel worthless?"
string_question_var1 <- "Do you have a spark in your life?"

get_grouped_bar_chart(raw_crosstab,title_text,string_question_var1,string_question_var2,svy_dd)
push_crosstab_table(raw_crosstab,"crosstab_psych_distress_sparks")
```
...
raw_crosstab <- cross_tab_df(svy_data,"eo","cy",svy_dd)
...
title_text <- "Experiencing repeated microaggressions harms mental health"
...
get_grouped_bar_chart(raw_crosstab,title_text,string_question_var1,string_question_var2,svy_dd)
push_crosstab_table(raw_crosstab,"crosstab_psych_distress_microaggressions")
```
...
raw_crosstab <- cross_tab_df(svy_data,"et","cy",svy_dd) 
...
string_question_var2 <- "How often during the past 30 days did you feel worthless?"
string_question_var1 <- "How often does poor quality health services get in the way of you living your best life?"

get_grouped_bar_chart(raw_crosstab,title_text,string_question_var1,string_question_var2,svy_dd)
push_crosstab_table(raw_crosstab,"crosstab_psych_distress_structural_racism")
```
...
raw_crosstab <- cross_tab_df(svy_data,"dl","co",svy_dd) 
...
get_grouped_bar_chart(raw_crosstab,title_text,string_question_var1,string_question_var2,svy_dd)
push_crosstab_table(raw_crosstab,"crosstab_self_efficacy_caring_relationships")
```

### Self-efficacy and hope - Cultural identity
```{r}
# Get crosstab of 2 questions, Self-efficacy and hope - Cultural identity   
raw_crosstab <- cross_tab_df(svy_data,"dz","co",svy_dd) 
title_text <- "Having a positive cultural identity encourages more hope for the future"
string_question_var2 <- "I feel hopeful when I think about my future."
string_question_var1 <- "My culture helps me feel good about who I am."

get_grouped_bar_chart(raw_crosstab,title_text,string_question_var1,string_question_var2,svy_dd)
push_crosstab_table(raw_crosstab,"crosstab_self_efficacy_cultural_identity")
```
...
raw_crosstab <- cross_tab_df(svy_data,"dm","co",svy_dd) 
...
get_grouped_bar_chart(raw_crosstab,title_text,string_question_var1,string_question_var2,svy_dd)
push_crosstab_table(raw_crosstab,"crosstab_self_efficacy_community_involvement")
```
...
```{r}
    ...
    # Set the x-axis title
    string_question_var2 <- str_wrap(string_question_var2, width = 90)
    # Reorder the x-axis labels 
    crosstab_df <- crosstab_df %>%
        mutate(response_var2 = reorder(response_var2, var2_number))
    # Wrap the x-axis labels at a reasonable width 
    ...
    # Set the legend title
    string_question_var1 <- str_wrap(string_question_var1, width = 10)
    # Reorder the legend labels 
    crosstab_df <- crosstab_df %>%
        mutate(response_var1 = reorder(response_var1, -var1_number))    
    # Set the bar colors
    ...
    caption_text <- str_wrap(paste0(
      ...
      ", Subcomponent: ", unique(crosstab_df$subcomponent_var2), ". ",
      "Data Source: Catalyst California calculations of Bold Vision Youth Thriving Survey, 2024."
    ), width = 140)  # Apply wrapping only once
    ...
    final_visual <- ggplot(crosstab_df, aes(fill=response_var1, y=rate, x=response_var2)) + 
      ...
      labs(title = title_text, caption = caption_text) +          # Set the chart title and caption
      theme_minimal() +
      theme(plot.margin = margin(10, 30, 10, 10),  # Reduce right margin to fit everything
        legend.position = "top",
