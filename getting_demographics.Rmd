...
```{r}
asian_aoic_total <- nrow(race_ethnicity_data%>%filter(race_asian==1))

# includes NA or where people did not respond to detailed asian question
asian_ethnicity_demographics_df_revised <- race_ethnicity_data %>%
 filter(race_asian==1)%>% # all asian alone or in combination people
  count(detailed_asian) %>%  # Count occurrences of each unique value in the column
  rename(count = n) %>%
  mutate(variable_name = 'Asian detailed ethnicity', percentage = count/asian_aoic_total*100) %>%
...
# total percentage every system involved
system_involved_yes<-svy_data %>% 
  count(q24)%>%
  rename(count = n) %>%
  mutate(variable_name = 'Ever system involved', percentage = count/total_respondents*100,
         variable="At any point system involved (q24)")%>%
  filter(q24==1)%>%
  select(variable_name, variable,everything(), -q24)

# system involved by type
# vars contains the column names and response values of a question domain (e.g. race)
vars<-svy_dd %>% 
...
df <- svy_data %>%
  ...
  mutate(across(where(is.character), ~ ifelse(!is.na(.), 1, 0))) %>% # Convert specific non-numeric values
  summarise(across(where(is.numeric), ~ sum(.x, na.rm = TRUE))) %>% # Sum up numeric columns, ignoring NA
  mutate(variable_name = 'System Involvement Type')
...
```{r}
# system involvement out of system involved youth
...
system_involved_demographics_df_2<-df %>% 
  pivot_longer(!variable_name,names_to='variable',values_to='count') %>%
  left_join(vars, by="variable") %>% 
  mutate(percentage = count/system_involved_total * 100) %>%
  select(variable_name, variable, response_1, count, percentage)
...
arrested_yes<-svy_data %>% 
  count(q25)%>%
  rename(count = n) %>%
  mutate(variable_name = 'Law Enforcement Involvement', percentage = count/total_respondents*100,
         variable="Ever been detained or arrested (q25)")%>%
  filter(q25==1)%>%
...
immigration_yes<-svy_data %>% 
  count(q27)%>%
  rename(count = n) %>%
  mutate(variable_name = 'Immigration-system impacted', percentage = count/total_respondents*100,
         variable="Temporary Protected Status, Asylum Status, Refugee Status, DACA, Non-Citizen Resident (q27)")%>%
  filter(q27==1)%>%
...
unhoused_yes<-svy_data %>% 
  count(q28)%>%
  rename(count = n) %>%
  mutate(variable_name = 'Housing Status', percentage = count/total_respondents*100,
         variable="Unhoused in the past year (q28)")%>%
  filter(q28==1)%>%
