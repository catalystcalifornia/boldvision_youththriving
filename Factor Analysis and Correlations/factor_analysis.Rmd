...
for (col in columns_to_reverse) {
  ...
  #small_svy_data[[col]] <- scale_max + 1 - small_svy_data[[col]]
  small_svy_data[[col]] <- ifelse(is.na(small_svy_data[[col]]),
                                NA,
                                scale_max + 1 - small_svy_data[[col]])
}
unique(small_svy_data$q10)
# for (col in columns_to_reverse) {
#   # Get the maximum scale value from the 'likert' column
#   scale_max <- as.numeric(sub("pt_scale", "", small_svy_dd$likert[small_svy_dd$variable == col]))
#   
#   # Reverse the scale and ensure numeric conversion
#   small_svy_data[[col]] <- ifelse(
#     is.na(small_svy_data[[col]]), 
#     NA, 
#     scale_max + 1 - as.numeric(small_svy_data[[col]])
#   )
# }
...
run_cfa <- function(model_name) {
  ...
  # AV 12/6/24: The if statement is commented out for now as we conduct more research on model fit measures
    ...
    loadings_summary <- parameterEstimates(fit.cat, standardized = TRUE) %>% 
      ...
      arrange(desc(Standardized)) # Reorder by Item in descending order

    # Create question_strings by matching 'Item' in loadings_summary with 'variable' in svy_dd
    # question_strings <- svy_dd %>%
    #   filter(variable %in% loadings_summary$Item) %>%
...
```{r, include=FALSE, echo=FALSE}
# Create the subcomponent and component CFA models
...
```{r, include=FALSE, echo=FALSE}
# Contains all the component models we want to run factor analysis on
component_models <- c(
  ...
  # component_safety_model, # component doesn't have enough at least 3 items, can't run factor analysis
  component_strong_minds_model,
  component_vibrant_communities_model
)

# Contains all the subcomponent models we want to run factor analysis on
# These are the models that have 3 or more question items per subcomponent or component
subcomponent_models <- c(
  # subcomponent_connectedness_model, # doesn't have enough at least 3 items
  # subcomponent_critical_action_model, # doesn't have enough at least 3 items
  ...
  # subcomponent_freedom_to_explore_the_self_model, # doesn't have enough at least 3 items
  # subcomponent_growth_mindset_model, # doesn't have enough at least 3 items
  # subcomponent_hope_for_the_future_model, # doesn't have enough at least 3 items
  subcomponent_microaggressions_model,
  # subcomponent_personal_safety_model, # doesn't have enough at least 3 items
  # subcomponent_positive_emotions_model, # doesn't have enough at least 3 items
  subcomponent_psychological_distress_model,
  # subcomponent_relationships_support_model, # doesn't have enough at least 3 items
  ...
  # subcomponent_self_efficacy_model, # doesn't have enough at least 3 items
  # subcomponent_sparks_model, # doesn't have enough at least 3 items
...
# Get factor loadings for all components
component_results <- lapply(component_models, run_cfa)
# Get factor loadings for all subcomponents
subcomponent_results <- lapply(subcomponent_models, run_cfa)
```