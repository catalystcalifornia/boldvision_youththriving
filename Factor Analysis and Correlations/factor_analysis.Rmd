...
run_cfa <- function(model_name) {
  ...
  # AV 12/6/24: The if statement is commented out for now as we conduct more research on model fit measures
    ...
    cat("Number of factor loadings meeting the threshold (>=", threshold_factor_loading, "):", 
        count_met_threshold, "out of", ... = nrow(loadings_summary), "factors,", short_model_name, "\n")
...
# all_models contains all the models we want to run factor analysis on
```{r}
all_models <- c(
...
```{r}
cfa_results <- lapply(all_models, run_cfa)
```
