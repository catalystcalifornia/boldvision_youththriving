...
get_fit_measures <- function(fitted_model){
  ...
  fit_measures_output <- fit_measures_output %>%
    ...
    select(fit_measures_met, everything())
...
get_sem_results <- function(fitted_model){
  ...
  select(c(rhs,std.lv,p_is_significant,pvalue, fit_measures_met)) %>%
  # Commented out for now to minimize info on the HTML output
  # select(c(lhs,rhs,std.lv,p_is_significant,fit_measures_met,pvalue,cfi,tli,rmsea,srmr)) %>%
