...
get_revised_response <- function(response) {
  ...
  if (grepl("^null$|^mixed$|^mixed race$|^multiracial$|^mixed/multiracial$|^Belizean$|^two or more$", response, ignore.case = TRUE))
...
svy_data_revised <- svy_data_clean %>%
  ...
  mutate(
    ...
    ba = case_when(
      grepl("^not_ba$", revised_response) ~ NA,
      grepl("^ba$", revised_response) ~ ba)) %>%
  ungroup() %>%
  mutate(  
