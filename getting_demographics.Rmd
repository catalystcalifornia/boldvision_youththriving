...
race_demographics_df <- race_ethnicity_data %>%
  summarise(
    ...
    race_aian_pct = sum(race_aian) / total_responses*100,
...
race_ethnicity_demographics_df_qa<-race_ethnicity_data %>%
   filter(detailed_race != "") %>%  # Remove any empty entries
  group_by(detailed_race)%>%
  summarise(count=n(),.groups='drop')%>%
  mutate(rate=count/sum(count)*100)

nh_race_demographics_df_qa<-race_ethnicity_data %>%
   filter(nh_race!="") %>%  # Remove any empty entries
  group_by(nh_race)%>%
  summarise(count=n(),.groups='drop')%>%
  mutate(rate=count/sum(count)*100)
...
asian_ethnicity_demographics_df_qa<-race_ethnicity_data %>%
   filter(detailed_asian != "") %>%  # Remove any empty entries
...
nhpi_demographics_df_qa<-race_ethnicity_data %>%
   filter(detailed_nhpi != "") %>%  # Remove any empty entries
  group_by(detailed_nhpi)%>%
  summarise(count=n(),.groups='drop')%>%
  mutate(rate=count/sum(count)*100)
...
vars<-svy_dd%>%filter(variable_name=='Gender')%>%select(variable,response_1)

df<-svy_data %>% select(all_of(vars$variable))%>%select(where(is.numeric))%>%
  summarise_all(sum,na.rm=TRUE)%>%
  mutate(variable_name='Gender')

df_long<-df%>%pivot_longer(!variable_name,names_to='response',values_to='count')%>%
  left_join(vars)
